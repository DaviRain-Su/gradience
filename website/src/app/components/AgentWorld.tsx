  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      streams.current = streams.current.filter(s => {
        ...
        const a = ents[s.from], b = ents[s.to];
        if (!a || !b) return false;
        const x = a.x + (b.x - a.x) * s.progress;
        const y = a.y + (b.y - a.y) * s.progress;
        const alpha = s.progress < 0.15 ? s.progress / 0.15
          : s.progress > 0.85 ? (1 - s.progress) / 0.15 : 1;
