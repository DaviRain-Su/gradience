...
class SoulMatchingEngine {
  ...
  async runMatching(probeId: string, options?: {
    ...
    const embeddingScore = this.calculateEmbeddingSimilarity(
      proberSoul.embedding,
      targetSoul.embedding
    );
  ...
  private async runA2AProbe(
    probeId: string,
    params: InitiateProbeParams
  ): Promise<void> {
    const probe = await this.getProbe(probeId);
