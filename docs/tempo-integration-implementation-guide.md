...
export class TempoProvider implements ChainProvider {
  ...
  constructor(config: TempoConfig) {
    ...
    this.mppContract = new ethers.Contract(
  ...
  async createMPPSession(
    ...
    const approveTx = await tokenContract.approve(
      this.mppContract.target,
      budget
    );
    await approveTx.wait();
    
    // 创建 session
    const tx = await this.mppContract.createSession(
      recipient,
      budget,
      duration
    );
  ...
  async streamPayment(
    ...
    const tx = await this.mppContract.streamPayment(
      sessionId,
      amount
    );
    await tx.wait();
   ...
   * 设置后自动按时间释放资金
   */
  async createAutoStream(
    config: StreamingConfig
  ): Promise<string> {
    // 创建 session
    const session = await this.createMPPSession(
      config.recipient,
      config.totalAmount,
      config.duration
    );
...
async function fullTaskWithTempo() {
  ...
  const payment = await hub.tempo!.agentAutonomousPay(
