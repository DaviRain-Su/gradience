...
export class MatchEngine {
    ...
    // 本地匹配算法
    async evaluateMatch(localSoul: SoulProfile, remoteDigest: SoulDigest): Promise<MatchEvaluation> {
        const scores = {
    ...
    private calculateSkillComplementarity(local: SoulProfile, remote: SoulDigest): number {
        ...
        // 互补性 = 双方都能提供对方需要的
        const complementarity = (localHasRemoteNeeds + remoteHasLocalNeeds) / (localSkills.size + remoteSkills.size);
...
export class PrivacyLayer {
    ...
    // 协商共享密钥
    async deriveSharedSecret(sessionId: string, remotePublicKey: Uint8Array): Promise<Uint8Array> {
    ...
    // 加密披露数据
    async encrypt(data: unknown, sharedSecret: Uint8Array): Promise<EncryptedData> {
    ...
    // 解密披露数据
    async decrypt(encrypted: EncryptedData, sharedSecret: Uint8Array): Promise<unknown> {
        const key = sharedSecret.slice(0, 32);
    ...
    // 生成 Soul.md 摘要
    async generateDigest(soulMd: string, level: DisclosureLevel): Promise<SoulDigest> {
...
export function createP2pSoulRouter(fsmFactory: HandshakeFSMFactory, discovery: DiscoveryService): Router {
    const router = Router();
