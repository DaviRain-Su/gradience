...
| T02 | 常量 + 错误码模块 | `constants.rs` 所有 **15 个**不可变常量（JUDGE_FEE_BPS/PROTOCOL_FEE_BPS/AGENT_FEE_BPS/CANCEL_FEE_BPS/FORCE_REFUND_DELAY/UNSTAKE_COOLDOWN/MAX_JUDGES_PER_POOL/MAX_CATEGORIES/MIN_SCORE/MAX_SCORE/MAX_REF_LEN/MAX_PROVIDER_LEN/MAX_MODEL_LEN/MAX_RUNTIME_LEN/MAX_VERSION_LEN）+ 1 个可配置常量（min_judge_stake）；`errors.rs` **29 个**错误码（6000-6041，分组编号含间隔：状态 6000-6005，权限 6010-6014，质押 6020-6025，验证 6030-6039，算术 6040，Token 6041） | T01 | 1h | P0 | 编译通过；15 个不可变常量类型正确；29 个错误码名称与规格 100% 一致 |
