...
function applicationPda(taskId: bigint, agent: PublicKey): [PublicKey, number] {
    const taskIdBytes = Buffer.allocUnsafe(8);
    taskIdBytes.writeBigUInt64LE(taskId);
    return PublicKey.findProgramAddressSync([Buffer.from('application'), taskIdBytes, agent.toBuffer()], PROGRAM_ID);
}

function submissionPda(taskId: bigint, agent: PublicKey): [PublicKey, number] {
    const taskIdBytes = Buffer.allocUnsafe(8);
    taskIdBytes.writeBigUInt64LE(taskId);
    return PublicKey.findProgramAddressSync([Buffer.from('submission'), taskIdBytes, agent.toBuffer()], PROGRAM_ID);
}
...
_文档维护方: Gradience Protocol Team_
