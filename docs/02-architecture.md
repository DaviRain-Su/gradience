...
| Helius | — | **默认 RPC 提供商**（99.99% 可用，<100ms）；Webhooks 推送 Program 事件给 Indexer（Managed 模式首选）；LaserStream gRPC 给 Judge Daemon 超低延迟触发；MCP Server（60+ 工具，与 Claude Code 集成）；TypeScript + Rust SDK | 是（可替换为 Triton，见下行） |
| Triton One | — | **企业级/Self-hosted RPC 备选**（[Project Yellowstone](https://docs.triton.one)）；**Dragon's Mouth** gRPC（Geyser-fed，Judge Daemon 事件流，可替代 Helius LaserStream）；**Fumarole** 可靠流（多节点 HA + 断线重连 + Consumer Group 水平扩展，Self-hosted Indexer 生产级首选，不丢事件）；**Old Faithful** 历史存档（Solana 创世至今全量数据，历史信誉查询）；**Steamboat** 自定义索引（getProgramAccounts ~30ms vs ~1700ms，JudgePool 查询加速） | 是（Helius 可替换） |
