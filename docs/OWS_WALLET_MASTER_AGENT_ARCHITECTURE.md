...
interface OWSWalletAPI {
    ...
    // 创建新 Agent 钱包
    createAgentWallet(agentName: string, permissions: AgentPermissions): Promise<AgentWallet>;

    // 修改权限
    updatePermissions(agentId: string, newPermissions: AgentPermissions): Promise<void>;
