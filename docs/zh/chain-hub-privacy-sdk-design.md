...
class ZKKYCModule {
  ...
  // 4. 刷新 ZK 凭证
  async refreshZKIdentity(
    oldIdentity: ZKIdentity
  ): Promise<ZKIdentity>;
  
  // 5. 撤销 ZK 身份
  async revokeZKIdentity(
    identity: ZKIdentity
  ): Promise<void>;
}
...
class IdentityCryptoModule {
  ...
  // 2. 加密消息给指定身份
  async encryptToIdentity(
    recipientId: string,
    message: Uint8Array
  ): Promise<EncryptedMessage>;
  
  // 3. 解密消息（需要 MPC 协作）
  async decryptFromIdentity(
...
class ChainHubPrivacySDK {
  ...
  async enableCloakedMode(
    ...
    // 1.1 发行 ZK 身份
    const zkIdentity = await this.zkKYC.issueZKIdentity(
      agentConfig.kycData,
      { attributesToProve: agentConfig.attributesToProve }
    );
    
    // 1.2 生成身份加密密钥
    const identityKeys = await this.identityCrypto.generateIdentityKeys(
      zkIdentity.did
    );
  ...
  async bidTaskCloaked(
    ...
    // 2.1 生成 ZK 证明（KYC + 声誉）
    const kycProof = await this.zkKYC.proveIdentityAttribute(
      cloakedAgent.zkIdentity,
      { name: 'kycVerified', operator: '==', value: true }
    );
  ...
  async sendAgentMessage(
    ...
    await this.identityCrypto.sendSecureMessage(
      channel,
      Buffer.from(message)
    );
  }
