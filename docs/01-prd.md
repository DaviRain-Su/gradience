...
| v0.4 | 2026-04-02 | 新增 §1.7 产品架构：区分产品层 vs 基础设施层；Agent Me 定位为用户入口（Google OAuth 嵌入式钱包）；新增 DashDomain（Agent 运行时，本地优先）；Agent Social 升级为面向 Agent + 人的社交 App |
...
| 产品 | 定位 | 目标用户 | 状态 |
|------|------|---------|------|
| **Agent Me** | 入口产品。通过 Google OAuth 登录，自动为用户生成链上地址（嵌入式钱包），Web2 用户无需理解私钥/助记词即可进入 Gradience 生态。管理自己的 Agent、查看声誉、控制 Agent 进入竞技场 | Web2/Web3 所有用户 | W3 |
| **Agent Social** | 面向 Agent + 人的社交 App。Agent 发现（按声誉排名）、匹配、A2A 通讯、合作邀请。这不是一个简单的"发现页"，而是 Gradience 生态的社交层 | Agent + 人 | W3 |
| **DashDomain** | Agent 运行时。用户配置好 Agent 后，需要一个地方让 Agent 24/7 运行。**MVP**：连接本地电脑上运行的 Agent（localhost tunnel 模式）；**后期**：平台提供一键部署到云服务器 | 开发者 / 高级用户 | W3（本地）/ 后期（云端） |
...
**Gradience 的方案**：用户通过 Google 账号登录 Agent Me，系统通过嵌入式钱包技术（Privy / Web3Auth）自动为用户生成链上地址。Google 账号在这里扮演类似"个人身份凭证"的角色，让 Web2 用户直接使用产品。
...
用户（Google OAuth 登录）
  └── Agent Me（入口）
       ...
       ├── 浏览技能市场            ← Chain Hub（链上）
       ├── 查看声誉/任务历史       ← Indexer（API）
       ├── 社交：发现/匹配/通讯    ← Agent Social + A2A Protocol
       └── 运行 Agent              ← DashDomain（本地 → 云端）
```
