...
export class SNSProvider implements Provider {
    ...
    constructor(config: SNSProviderConfig = {}) {
        ...
        const connectionUrl =
            typeof config.connection === 'string' ? config.connection : 'https://api.mainnet-beta.solana.com';

        this.connection =
            config.connection instanceof Connection ? config.connection : new Connection(connectionUrl, 'confirmed');
    ...
    async resolve(domain: string): Promise<string | null> {
        if (!this.supports(domain)) {
            throw new ProviderError(`Domain ${domain} is not supported by SNS provider`, this.name, domain);
        }

        const domainName = domain.replace('.sol', '');

        if (!this.isValidSolDomain(domainName)) {
            throw new ProviderError(`Invalid .sol domain format: ${domain}`, this.name, domain);
        }
        ...
        try {
            ...
            const timeoutPromise = new Promise<never>((_, reject) => {
                setTimeout(() => {
                    reject(new ProviderError(`SNS resolution timeout after ${this.timeout}ms`, this.name, domain));
                }, this.timeout);
            });
    ...
    async reverse(address: string): Promise<string | null> {
        try {
            ...
            const timeoutPromise = new Promise<never>((_, reject) => {
                setTimeout(() => {
                    reject(new ProviderError(`SNS reverse lookup timeout after ${this.timeout}ms`, this.name));
                }, this.timeout);
            });
