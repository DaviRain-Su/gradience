...
describe('DomainResolver', () => {
    ...
    describe('resolve', () => {
        ...
        it('should throw ValidationError for invalid domain', async () => {
            await expect(resolver.resolve('invalid-domain')).rejects.toThrow(ValidationError);
        });

        it('should throw ValidationError for unsupported domain', async () => {
            await expect(resolver.resolve('test.xyz')).rejects.toThrow(ValidationError);
        });
        ...
        it('should handle ENS domains (should throw not implemented)', async () => {
            await expect(resolver.resolve('alice.eth')).rejects.toThrow(ProviderError);
        });
    });
...
describe('ENSProvider', () => {
    ...
    describe('resolve', () => {
        it('should throw ProviderError (not implemented)', async () => {
            await expect(provider.resolve('alice.eth')).rejects.toThrow(ProviderError);
        });
    });
});
