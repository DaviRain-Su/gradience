 ...
 */
export function generateMessageId(sender: string, timestamp: number, sequence: number): string {
 ...
 */
export async function withRetry<T>(fn: () => Promise<T>, max = 3, delay = 100): Promise<T> {
    let lastErr: unknown;
...
export function isTaskOffer(msg: A2AMessage): msg is A2AMessage<TaskOfferPayload> {
    return msg.messageType === GradienceMessageType.TaskOffer;
}

export function isTaskResult(msg: A2AMessage): msg is A2AMessage<TaskResultPayload> {
    return msg.messageType === GradienceMessageType.TaskResult;
}

export function isJudgeVerdict(msg: A2AMessage): msg is A2AMessage<JudgeVerdictPayload> {
    return msg.messageType === GradienceMessageType.JudgeVerdict;
}

export function isPaymentConfirmation(msg: A2AMessage): msg is A2AMessage<PaymentConfirmationPayload> {
