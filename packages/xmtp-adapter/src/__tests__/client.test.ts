...
const mockSend = jest.fn().mockResolvedValue({ id: "xmtp-1", sent: new Date(), senderAddress: "0xsender", conversation: { topic: "t1", peerAddress: "0xpeer" }, content: new Uint8Array() });
...
describe("XMTPClient.sendMessage", () => {
  ...
  it("throws when not connected", async () => {
    const client = new XMTPClient();
    await expect(
      client.sendMessage(PEER, GradienceMessageType.TaskOffer, OFFER),
...
describe("XMTPClient.streamMessages", () => {
  it("calls callback with decoded messages", async () => {
    const unsigned = buildUnsignedMessage(
      mockXMTPInstance.address,
      PEER,
      GradienceMessageType.TaskResult,
      { parentTaskId: "t1", subtaskId: 1, deliverable: "done", bidAmount: BigInt(100) },
    );
    ...
    const stop = await client.streamMessages((m) => { received.push(m); });
