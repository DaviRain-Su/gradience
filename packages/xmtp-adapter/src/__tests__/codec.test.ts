...
describe("GradienceCodec", () => {
  ...
  it("returns null for an unknown message type", () => {
    const wire = JSON.stringify({ v: 1, messageType: "unknown_type", sender: SENDER, recipient: RECIPIENT, payload: {}, timestamp: 0, signature: "", id: "x" });
    const bytes = new TextEncoder().encode(wire);
    expect(GradienceCodec.decode(bytes)).toBeNull();
  });

  it("returns null when codec version is not 1", () => {
    const wire = JSON.stringify({ v: 2, messageType: GradienceMessageType.TaskOffer, sender: SENDER, recipient: RECIPIENT, payload: {}, timestamp: 0, signature: "", id: "x" });
    const bytes = new TextEncoder().encode(wire);
    expect(GradienceCodec.decode(bytes)).toBeNull();
  });
...
describe("buildSigningInput", () => {
  ...
  it("changes when any field changes", () => {
    ...
    const diffTs = buildSigningInput(SENDER, RECIPIENT, GradienceMessageType.TaskOffer, OFFER_PAYLOAD, 9999, "id-1");
