...
export type { LLMConfig, LLMProvider, LLMConfigWithFallback, FallbackMode } from '../llm-config.js';
...
export class MatchingEngine {
    ...
    async findMatches(
        ...
        const embeddingMatches = await this.embeddingMatcher.findTopMatches(sourceProfile, candidateProfiles);
        ...
        for (let i = 0; i < topK; i++) {
            ...
            let llmAnalysis;
            if (runLLM) {
                llmAnalysis = await this.llmAnalyzer.analyzeCompatibility(sourceProfile, targetProfile);
            } else {
    ...
    async analyzeMatch(
        ...
        // Run LLM analysis
        const llmAnalysis = await this.llmAnalyzer.analyzeCompatibility(sourceProfile, targetProfile, probeSession);
