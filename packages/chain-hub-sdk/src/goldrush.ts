...
export class GoldRushClient {
  ...
  combineWithReputation(
    ...
    const reputationComponent = clampScore(
      Math.round(
        reputation.avgScore * 0.72 +
          Math.min(reputation.completed, 40) * 0.45 +
          normalizedWinRate * 100 * 0.1
      )
    );
...
function buildMetricsFromInputs(
  ...
  const riskScore = clampScore(
    Math.round(concentrationRisk * 0.4 + approvalRisk * 0.25 + txPatternRisk * 0.35)
  );
