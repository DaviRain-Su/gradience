...
import { createRealTradingHandlers } from '../src/handlers/index.js';
...
console.log(
    `   createTradingHandlers: ${tradingHandlers.size} handlers ${missingFromTrading.length > 0 ? '(missing: ' + missingFromTrading.join(', ') + ')' : '✓'}`,
);
console.log(
    `   createRealTradingHandlers: ${realHandlers.size} handlers ${missingFromReal.length > 0 ? '(missing: ' + missingFromReal.join(', ') + ')' : '✓'}`,
);
