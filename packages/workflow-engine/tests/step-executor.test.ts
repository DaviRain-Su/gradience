...
function createDelayedHandler(result: Record<string, unknown>, delay: number): ActionHandler {
    return {
        execute: vi.fn().mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(result), delay))),
    };
}
...
describe('StepExecutor', () => {
    describe('execute()', () => {
        ...
        it('should parse template variables in params', async () => {
            ...
            expect(handler.execute).toHaveBeenCalledWith('solana', { message: 'parsed-value' }, expect.any(Object));
        });
