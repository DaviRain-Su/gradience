...
async function main() {
    ...
    try {
        ...
        const reviewSig = await sdk.reviewWorkflow(workflowId, 5, 'Excellent workflow! Easy to use.');
        ...
        const updateSig = await sdk.updateWorkflow(workflowId, 'ipfs://QmUpdatedExample456');
        ...
        console.log(
            '   Explorer:',
            `https://explorer.solana.com/address/${sdk.getWorkflowAddress(workflowId).toBase58()}?cluster=devnet`,
        );
    } catch (error) {
        console.error('\n❌ Error:', error);
        throw error;
