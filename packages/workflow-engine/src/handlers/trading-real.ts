...
import { TritonCascadeClient, createConfigFromEnv, type SendTransactionOptions } from '../triton-cascade/index.js';
...
export function createRealBridgeHandler(
    ...
    return {
        async execute(
            ...
            const { fromChain, toChain, token, amount, recipient, signer } = params as {
...
export function createRealUnstakeHandler(
    ...
    return {
        async execute(
            ...
            const { stakeAccount, amount, signer } = params as {
                stakeAccount: string;
                amount?: string;
                signer: Signer;
            };
...
export function createRealRepayHandler(
    ...
    return {
        async execute(
            ...
            const { token, amount, signer } = params as {
                token: string;
                amount: string;
                signer: Signer;
            };
