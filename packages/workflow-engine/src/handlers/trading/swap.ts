...
import type { SwapHandlerConfig, JupiterQuote, JupiterSwapResponse } from './types.js';
 ...
 */
export function createRealSwapHandler(config: SwapHandlerConfig = {}): ActionHandler {
    const {
    ...
    return {
        async execute(chain: SupportedChain, params: Record<string, unknown>): Promise<Record<string, unknown>> {
            if (chain !== 'solana') {
            ...
            try {
                ...
                if (!quoteResponse || !quoteResponse.ok) {
                    throw new Error(
                        `Jupiter quote failed after 3 retries: ${(lastError as Error)?.message || quoteResponse?.statusText}`,
                    );
                }
