...
export function createDefaultConfig(
  ...
  const rpcEndpoint =
    network === 'mainnet'
      ? ENDPOINTS.CASCADE_MAINNET
      : ENDPOINTS.CASCADE_DEVNET;
...
export function createConfigFromEnv(): TritonCascadeConfig {
  const network =
    (process.env.SOLANA_NETWORK as SolanaNetwork) ||
  ...
  if (process.env.TRITON_CONNECTION_TIMEOUT_MS) {
    config.connectionTimeoutMs = parseInt(
      process.env.TRITON_CONNECTION_TIMEOUT_MS,
      10
    );
  }

  if (process.env.TRITON_CONFIRMATION_TIMEOUT_MS) {
    config.confirmationTimeoutMs = parseInt(
      process.env.TRITON_CONFIRMATION_TIMEOUT_MS,
      10
    );
  }
  ...
  if (process.env.TRITON_FIXED_PRIORITY_FEE_LAMPORTS) {
    config.fixedPriorityFeeLamports = parseInt(
      process.env.TRITON_FIXED_PRIORITY_FEE_LAMPORTS,
      10
    );
  }

  if (process.env.TRITON_MAX_CONCURRENT_TRANSACTIONS) {
    config.maxConcurrentTransactions = parseInt(
      process.env.TRITON_MAX_CONCURRENT_TRANSACTIONS,
      10
    );
  }
...
export function validateConfig(
  config: Partial<TritonCascadeConfig>
): TritonCascadeConfig {
  const errors: string[] = [];
  ...
  if (
    ...
    errors.push(
      `connectionTimeoutMs must be between 1000 and 300000, got ${config.connectionTimeoutMs}`
    );
  }
  ...
  if (
    ...
    (config.confirmationTimeoutMs < 5000 ||
      config.confirmationTimeoutMs > 600000)
  ) {
    errors.push(
      `confirmationTimeoutMs must be between 5000 and 600000, got ${config.confirmationTimeoutMs}`
    );
  }
  ...
  if (
    ...
    (config.maxConcurrentTransactions < 1 ||
      config.maxConcurrentTransactions > 100)
  ) {
    errors.push(
      `maxConcurrentTransactions must be between 1 and 100, got ${config.maxConcurrentTransactions}`
    );
  }

  // Validate priority fee strategy
  if (
    config.priorityFeeStrategy &&
    !['auto', 'fixed', 'none'].includes(config.priorityFeeStrategy)
  ) {
  ...
  if (
    config.fixedPriorityFeeLamports !== undefined &&
    config.fixedPriorityFeeLamports < 0
  ) {
    errors.push(
      `fixedPriorityFeeLamports must be non-negative, got ${config.fixedPriorityFeeLamports}`
    );
  }
...
export function mergeConfig(
  partial: Partial<TritonCascadeConfig>
): TritonCascadeConfig {
...
export function getJitoBlockEngineUrl(network: SolanaNetwork): string {
  return network === 'mainnet'
    ? ENDPOINTS.JITO_MAINNET_BLOCK_ENGINE
    : ENDPOINTS.JITO_DEVNET_BLOCK_ENGINE;
}
