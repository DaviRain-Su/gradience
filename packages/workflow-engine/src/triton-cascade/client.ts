  ...
  constructor(config?: Partial<TritonCascadeConfig>) {
    ...
    if (this.config.enableJitoBundle) {
      const jitoUrl =
        ...
        getJitoBlockEngineUrl(this.config.network);
  ...
  async sendTransaction(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.log(
          `[TritonCascade] Retry ${attempt + 1}/${maxRetries} after ${delay}ms`
        );
  ...
  private async submitTransaction(
    ...
    if (request.useJitoBundle && this.jitoClient) {
      try {
        ...
        if (jitoAvailable) {
          const bundleResult = await this.jitoClient.submitBundle(
            [request.transaction],
 ...
 */
export function createTritonCascadeClient(
