...
export class TransactionQueue {
     ...
     */
    add(request: CascadeTransactionRequest, retryCount = 0): Promise<import('./types.js').CascadeTransactionResponse> {
        // Check for duplicate signature
        if (this.signatures.has(request.signature)) {
            return Promise.reject(new Error(`Transaction with signature ${request.signature} already in queue`));
        }
    ...
    next(): TransactionQueueItem | undefined {
        ...
        const readyItems = this.queue.filter((item) => !item.nextRetryAt || item.nextRetryAt <= now);
    ...
    private calculateRetryDelay(retryCount: number): number {
        ...
        const exponentialDelay = this.options.retryBackoffBaseMs * Math.pow(2, retryCount);
