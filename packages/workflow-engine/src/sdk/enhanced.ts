...
export class EnhancedWorkflowSDK extends EventEmitter {
    ...
    async batchGet(workflowIds: string[]): Promise<BatchOperationResult<GradienceWorkflow | null>> {
        ...
        for (const chunk of chunks) {
            const results = await Promise.allSettled(chunk.map((id) => this.get(id)));
    ...
    async batchPurchase(
        workflowIds: string[],
    ): Promise<BatchOperationResult<import('./marketplace.js').PurchaseResult>> {
        const startTime = Date.now();
    ...
    async review(
        ...
        rating: number,
        comment: string,
    ): Promise<import('./marketplace.js').ReviewResult> {
        await this.withRateLimit();
    ...
    ): Promise<import('./marketplace.js').UpdateResult> {
        await this.withRateLimit();
