...
async function sendInstruction(instruction: Instruction, signer: TransactionSigner, rpc: RpcClient): Promise<string> {
...
export class SolanaWorkflowSDK {
     ...
     */
    async initialize(treasury: Address, upgradeAuthority: Address): Promise<string> {
        const instruction = await createInitializeInstruction(this.payer.address, treasury, upgradeAuthority);
     ...
     */
    async createWorkflow(workflow: GradienceWorkflow, workflowId: Address): Promise<string> {
        ...
        const params: CreateWorkflowParams = {
            ...
            priceAmount: workflow.pricing.oneTimePrice?.amount ? BigInt(workflow.pricing.oneTimePrice.amount) : 0n,
            creatorShare: workflow.revenueShare?.creator || 0,
            isPublic: workflow.isPublic,
        };

        const instruction = await createCreateWorkflowInstruction(this.payer.address, params);
        return sendInstruction(instruction, this.payer, this.rpc);
    }
     ...
     * Purchase a workflow
     */
    async purchaseWorkflow(workflowId: Address, accessType: number = 0): Promise<string> {
        const instruction = await createPurchaseWorkflowInstruction(this.payer.address, workflowId, accessType);
        return sendInstruction(instruction, this.payer, this.rpc);
    }
     ...
     */
    async purchaseWorkflowWithPayment(workflowId: Address, author: Address, accessType: number = 0): Promise<string> {
    ...
    async recordExecution(workflowId: Address): Promise<string> {
        const instruction = await createRecordExecutionInstruction(this.payer.address, workflowId);
        return sendInstruction(instruction, this.payer, this.rpc);
    }
     ...
     * Review a workflow
     */
    async reviewWorkflow(workflowId: Address, rating: number, comment: string): Promise<string> {
        ...
        const instruction = await createReviewWorkflowInstruction(this.payer.address, workflowId, rating, commentHash);
        return sendInstruction(instruction, this.payer, this.rpc);
    }
     ...
     */
    async updateWorkflow(workflowId: Address, newContentHash: string): Promise<string> {
        const contentHash = new Uint8Array(64);
        const hashBytes = Buffer.from(newContentHash.slice(7), 'hex');
        contentHash.set(hashBytes, 0);

        const instruction = await createUpdateWorkflowInstruction(this.payer.address, workflowId, contentHash);
        return sendInstruction(instruction, this.payer, this.rpc);
    }
    ...
    async deactivateWorkflow(workflowId: Address): Promise<string> {
        const instruction = await createDeactivateWorkflowInstruction(this.payer.address, workflowId);
        return sendInstruction(instruction, this.payer, this.rpc);
    }
    ...
    async activateWorkflow(workflowId: Address): Promise<string> {
        const instruction = await createActivateWorkflowInstruction(this.payer.address, workflowId);
        return sendInstruction(instruction, this.payer, this.rpc);
    }
    ...
    async deleteWorkflow(workflowId: Address): Promise<string> {
        const instruction = await createDeleteWorkflowInstruction(this.payer.address, workflowId);
        return sendInstruction(instruction, this.payer, this.rpc);
    }
     ...
     */
    async getWorkflow(workflowId: Address): Promise<OnChainWorkflowMetadata | null> {
        ...
        try {
            ...
            const hexSlice = (start: number, end: number) => Buffer.from(data.slice(start, end)).toString('hex');
 ...
 */
export function createSolanaWorkflowSDK(config: SolanaSDKConfig): SolanaWorkflowSDK {
