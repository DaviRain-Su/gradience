...
export async function generateWeeklyReport(
  ...
  const newRules = allRules.filter(r =>
    r.createdAt >= weekStart && r.createdAt <= weekEnd
  );

  // Get rules archived this week (status changed to deprecated)
  const archivedRules = allRules.filter(r =>
    r.status === 'deprecated' && r.updatedAt >= weekStart && r.updatedAt <= weekEnd
  );

  // Get expiring soon (next 7 days)
  const expiringSoon = checks.filter(c =>
    !c.isExpired && c.daysUntilExpiry > 0 && c.daysUntilExpiry <= 7
  );

  // Get recently expired (within last 7 days)
  const recentlyExpired = checks.filter(c =>
    c.isExpired && Math.abs(c.daysUntilExpiry) <= 7
  );
  ...
  // Get unused rules
  const unusedRules = allRules.filter(r =>
    r.applyCount === 0 && r.status !== 'deprecated'
  );
  ...
  for (const check of checks) {
    ...
    switch (recommendation) {
      ...
      case 'archive':
        ...
        break;
      case 'review':
        reason = check.lastAppliedDaysAgo !== undefined
...
export function compareReports(
  ...
  const currentActive = new Set(
    current.topAppliedRules.map(r => r.rule.id)
  );
