...
import type { ExpirationCheck, NotificationConfig, NotificationChannel, WeeklyReport } from './types.js';
...
export class RuleNotifier {
    ...
    private async sendToSlack(notification: Notification): Promise<void> {
        ...
        const color =
            notification.severity === 'critical' ? 'danger' : notification.severity === 'warning' ? 'warning' : 'good';
