...
import type { CursorRule, RuleStorage, NotificationConfig, WeeklyReport } from './types.js';
...
export class CursorRulesManager {
     ...
     */
    async addRule(
        rule: Omit<CursorRule, 'id' | 'createdAt' | 'updatedAt' | 'applyCount' | 'status'>,
    ): Promise<CursorRule> {
        const now = new Date();
    ...
    async getStats(): Promise<{
        ...
        const mostUsedRule =
            rules.length > 0 ? rules.reduce((max, r) => (r.applyCount > max.applyCount ? r : max)) : undefined;
