...
export class WormholeAdapter implements ProtocolAdapter {
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            this.options.logger.log(
                `[WormholeAdapter] Initialized on ${this.options.sourceChain} (Chain ID: ${this.options.sourceChainId})`,
            );
        } catch (error) {
            this.options.logger.error('[WormholeAdapter] Initialization failed:', error);
            throw new Error(
                `Failed to initialize Wormhole adapter: ${error instanceof Error ? error.message : String(error)}`,
                { cause: error },
            );
        }
    }
     ...
     * @returns Fee quote
     */
    async quote(payload: string, consistencyLevel?: number): Promise<WormholeFeeQuote> {
        if (this.options.demoMode) {
            // Demo fee estimation
     ...
     */
    async fetchVAA(emitterChain: number, emitterAddress: string, sequence: bigint): Promise<VAA | null> {
    ...
    private async connectCoreBridge(): Promise<void> {
        ...
        try {
            ...
            throw new Error(
                `Failed to connect to Wormhole core bridge: ${error instanceof Error ? error.message : String(error)}`,
                { cause: error },
            );
        }
    }
    ...
    private simulateSend(payload: string, message: WormholeMessage): BridgeResult {
        this.options.logger.log(
            `[WormholeAdapter] [DEMO] Sending message to Solana chain (Chain ID: ${this.options.solanaChainId})`,
        );
        this.options.logger.log(`[WormholeAdapter] [DEMO] Payload size: ${payload.length} bytes`);
        this.options.logger.log(
            `[WormholeAdapter] [DEMO] Consistency level: ${message.consistencyLevel ?? DEFAULT_CONSISTENCY_LEVEL}`,
        );
    ...
    private mapWormholeStatusToBridgeStatus(whStatus: WormholeMessageStatus): 'pending' | 'completed' | 'failed' {
        switch (whStatus) {
            case 'signed':
            case 'delivered':
                return 'completed';
        ...
        this.pollInterval = setInterval(async () => {
            ...
            for (const [messageId, receipt] of entries) {
                if (receipt.status === 'pending' || receipt.status === 'awaiting_signatures') {
                    try {
                        const vaa = await this.fetchVAA(receipt.emitterChain, receipt.emitterAddress, receipt.sequence);
