...
export const LZ_DVNS = {
    ...
    POLYGON: ['0x23DE2FE532d90411B9bFe2af49F49C2aF77E6211', '0xD56e4E23c5E29828B88F4E9b00B302e75be52dD3'],
    // Testnet DVNs
    ETHEREUM_TESTNET: ['0x8eebf8b423B64b9E46427bf9a0bE05170bA6e6E0'],
} as const;
...
export class LayerZeroAdapter implements ProtocolAdapter {
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            this.options.logger.log(
                `[LayerZeroAdapter] Initialized on ${this.options.sourceChain} (EID: ${this.options.sourceEid})`,
            );
        } catch (error) {
            this.options.logger.error('[LayerZeroAdapter] Initialization failed:', error);
            throw new Error(
                `Failed to initialize LayerZero adapter: ${error instanceof Error ? error.message : String(error)}`,
                { cause: error },
            );
        }
    }
     ...
     * @returns Fee quote
     */
    async quote(payload: string, options?: { gasLimit?: bigint; value?: bigint }): Promise<LayerZeroFeeQuote> {
        if (this.options.demoMode) {
            // Demo fee estimation
        ...
        try {
            ...
            // Call quote function
            const fee = await this.callQuote(this.options.solanaEid, payload, lzOptions, false);
    ...
    private async connectEndpoint(): Promise<void> {
        ...
        try {
            ...
            const nonce = await contract.getOutboundNonce(this.options.solanaEid, this.options.oAppAddress);

            this.options.logger.log(`[LayerZeroAdapter] Connected to endpoint, current nonce: ${nonce}`);
        } catch (error) {
            throw new Error(
                `Failed to connect to LayerZero endpoint: ${error instanceof Error ? error.message : String(error)}`,
                { cause: error },
            );
        }
    }
    ...
    private async sendWithRetry(message: CrossChainReputationMessage): Promise<BridgeResult> {
        ...
        throw new Error(`Failed to send after ${this.retryConfig.maxRetries + 1} attempts: ${lastError?.message}`);
    }

    private async sendViaLayerZero(message: CrossChainReputationMessage): Promise<BridgeResult> {
        // Sign the message
    ...
    private simulateSend(payload: string): BridgeResult {
        this.options.logger.log(
            `[LayerZeroAdapter] [DEMO] Sending message to Solana chain (EID: ${this.options.solanaEid})`,
        );
        ...
        const guid = `0x${Array.from({ length: 64 }, () => Math.floor(Math.random() * 16).toString(16)).join('')}`;
    ...
    private async signMessage(message: CrossChainReputationMessage): Promise<CrossChainReputationMessage> {
        if (this.options.demoMode) {
            // Simulate signature
...
export function createEthereumAdapter(
    options: Omit<LayerZeroAdapterOptions, 'sourceChain' | 'sourceEid' | 'endpointAddress'>,
): LayerZeroAdapter {
