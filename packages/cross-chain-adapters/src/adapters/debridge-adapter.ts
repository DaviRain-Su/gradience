...
export class DeBridgeAdapter implements ProtocolAdapter {
    ...
    constructor(options: DeBridgeAdapterOptions) {
        this.options = {
            ...
            apiUrl:
                options.apiUrl ??
                (options.network === 'testnet' || options.network === 'devnet'
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            this.options.logger.log(
                `[DeBridgeAdapter] Initialized on ${this.options.sourceChain} (Chain ID: ${this.options.sourceChainId})`,
            );
        } catch (error) {
            this.options.logger.error('[DeBridgeAdapter] Initialization failed:', error);
            throw new Error(
                `Failed to initialize DeBridge adapter: ${error instanceof Error ? error.message : String(error)}`,
                { cause: error },
            );
        }
    }
     ...
     */
    async lockAsset(tokenAddress: string, amount: string, targetTokenAddress: string): Promise<BridgeResult> {
        const message: DeBridgeMessage = {
            version: '1.0',
     ...
     * @returns Fee quote
     */
    async quote(payload: string, options?: { isTokenBridge?: boolean; amount?: string }): Promise<DeBridgeFeeQuote> {
        if (this.options.demoMode) {
            // Demo fee estimation
    ...
    async checkSubmissionStatus(submissionId: string): Promise<SubmissionStatus | null> {
        try {
            const response = await fetch(`${this.options.apiUrl}/api/Submission/${submissionId}`, {
    ...
    private async initializeSdk(): Promise<void> {
        ...
        try {
            ...
            this.gateContract = new Contract(this.options.gateAddress, GATE_ABI, this.wallet || (this.provider as any));
    ...
    private async connectGate(): Promise<void> {
        ...
        try {
            ...
            throw new Error(
                `Failed to connect to DeBridge Gate: ${error instanceof Error ? error.message : String(error)}`,
                { cause: error },
            );
        }
    }
    ...
    private simulateSend(payload: string, feeQuote: DeBridgeFeeQuote, message: DeBridgeMessage): BridgeResult {
        void message;
        this.options.logger.log(`[DeBridgeAdapter] [DEMO] Sending message via DeBridge`);
        this.options.logger.log(
            `[DeBridgeAdapter] [DEMO] Source: ${this.options.sourceChain} (${this.options.sourceChainId})`,
        );
