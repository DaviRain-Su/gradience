...
class ChainHubSocial {
    ...
    async initialize(): Promise<void> {
        ...
        console.log('[ChainHubSocial] Initialized with protocols:', this.router.getAvailableProtocols());
    }
    ...
    async discoverPeers(filters: {
        ...
        return uniquePeers.filter((peer) => {
            ...
            if (filters.capabilities && !filters.capabilities.some((c) => peer.capabilities.includes(c))) {
                return false;
            }
            return true;
        });
    ...
    async probe(params: {
        ...
        for (let turn = 0; turn < params.probeConfig.maxTurns; turn++) {
            ...
            // 等待回复
            const response = await this.waitForResponse(sessionId, turn, params.probeConfig.timeoutMs);
    ...
    private async waitForAcceptance(sessionId: string, timeoutMs: number): Promise<boolean> {
        return new Promise((resolve) => {
            ...
            const unsubscribe = this.router.subscribe(async (message) => {
                if (message.payload?.sessionId === sessionId && message.type === 'task_accept') {
     ...
     * 等待特定轮次的回复
     */
    private async waitForResponse(sessionId: string, turn: number, timeoutMs: number): Promise<A2AMessage | null> {
        return new Promise((resolve) => {
