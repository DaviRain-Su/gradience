...
class A2ACommerce {
    ...
    async publishCapability(params: {
        ...
        // 2. 检查发布者声誉
        const reputation = await this.hub.solana.getReputation(params.agentCard.creator);
    ...
    async executeTrade(tradeId: string): Promise<ExecutionResult> {
        ...
        try {
            // 1. 获取 Workflow
            const workflow = await this.hub.workflow.loadWorkflow(trade.item.execution.workflowId!);
    ...
    private async calculateJudgeScore(params: {
        ...
        for (const dimension of params.criteria.dimensions) {
            switch (dimension) {
                ...
                case 'output_quality':
                    // 可以结合 AI 评估
                    scores[dimension] =
                        params.evidence.metrics?.qualityScore || (await this.aiEvaluateQuality(params.evidence));
                    break;
        ...
        // 加权平均
        const total =
            params.criteria.dimensions.reduce((sum, dim, idx) => {
