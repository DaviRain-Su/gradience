...
**Important**: The forward address _for a given chain_ must match the reverse record on that chain's reverse registrar.
...
function UserProfile() {
    ...
    return (
        <div className="flex items-center gap-2">
            <img src={avatar || '/fallback-avatar.svg'} className="w-8 h-8 rounded-full" />
...
function AddressInput() {
    ...
    return (
        <div>
            <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="vitalik.eth or 0x..." />
...
export async function getENSProfile(addressOrName: string): Promise<ENSProfile> {
    ...
    if (name) {
        ...
        const [avatar, twitter, github, discord, email, description, url] = await Promise.all([
...
async function routeA2AMessage(message: A2AMessage) {
    const [fromAddress, toAddress] = await Promise.all([resolveAddress(message.from), resolveAddress(message.to)]);
...
_This document should be updated as ENSv2 rolls out and the ecosystem evolves._
