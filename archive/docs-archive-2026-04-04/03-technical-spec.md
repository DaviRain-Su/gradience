...
| `runtime_env` | RuntimeEnv | 各字段非空；provider.len() ≤ 32；model.len() ≤ 64；runtime.len() ≤ 32；version.len() ≤ 32 | 运行时环境声明。Borsh 反序列化不校验 String 长度上限——只要账户空间足够就能存入超长字符串。因此**必须**在指令处理函数中手动校验每个字段长度 ≤ 对应 MAX\_\*\_LEN 常量，防止恶意写入超大数据。 |
...
| `taskCategory` | U8       | 0-7                                  | 任务类别（与 §3.1 CATEGORY\_\* 常量一致）           |
