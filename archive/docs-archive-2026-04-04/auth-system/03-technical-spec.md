...
export class GoogleWalletAuth {
  ...
  private async createNewWallet(
    ...
    // 3. 加密存储方案
    const encryptedData = await this.encryptPrivateKey(
      privateKey, 
...
export class MPCAuth {
  ...
  async createMPCWallet(userId: string): Promise<MPCWallet> {
    // 生成三个分片
    const [clientShare, serverShare, backupShare] = 
      await this.mpc.generateKeyShares();
  ...
  async signTransaction(
    ...
    // 2. 两方共同签名（无需重构完整私钥）
    const signature = await this.mpc.sign(tx, [
      clientShare, 
      serverShare
    ]);
