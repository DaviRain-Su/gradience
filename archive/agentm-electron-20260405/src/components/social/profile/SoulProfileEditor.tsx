...
export function SoulProfileEditor({ initialProfile, onSave, onCancel }: SoulProfileEditorProps) {
    ...
    const [privacyLevel, setPrivacyLevel] = useState<PrivacyLevel>(
        initialProfile?.boundaries?.privacyLevel || 'public',
    );
    const [maxConversationLength, setMaxConversationLength] = useState(
        initialProfile?.boundaries?.maxConversationLength || 15,
    );
        ...
        <div className="max-w-4xl mx-auto p-6 space-y-8">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    {onCancel && (
                        <button onClick={onCancel} className="px-4 py-2 text-gray-400 hover:text-white transition">
                            Cancel
                        </button>
                    )}
                    <button
            ...
            <div className="w-full px-3 py-2 bg-gray-800 rounded-lg border border-gray-700 focus-within:border-blue-500">
                <div className="flex flex-wrap gap-2 mb-2">
                    {tags.map((tag, i) => (
                        ...
                        >
                            {tag}
                            <button onClick={() => removeTag(i)} className="hover:text-blue-300">
                                ×
                            </button>
                        </span>
                    ))}
