...
export function FollowerCount({
    ...
    const renderCount = (count: number, label: string, clickable: boolean, onClick?: () => void) => {
        if (horizontal) {
          ...
          `}
                >
                    <span className={sizeClasses[size].number}>{loading ? '-' : formatNumber(count)}</span>
        ...
        `}
            >
                <span className={`${verticalSizeClasses[size]} font-bold`}>{loading ? '-' : formatNumber(count)}</span>
    ...
    return (
        <div className={`${containerClasses} ${className}`}>
            ...
            {showBoth && horizontal && <span className="text-gray-700">·</span>}
...
export function CompactFollowerCount({ followers, following, className = '' }: CompactFollowerCountProps) {
