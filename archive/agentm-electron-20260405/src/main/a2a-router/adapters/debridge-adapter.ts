...
export class DebridgeAdapter implements ProtocolAdapter {
     ...
     */
    async syncReputation(reputationData: DebridgeMessage['reputationData']): Promise<BridgeResult> {
        const message: DebridgeMessage = {
            version: '1.0',
    ...
    async checkSubmissionStatus(submissionId: string): Promise<SubmissionStatus | null> {
        try {
            const response = await fetch(`https://api.debridge.finance/api/Submission/${submissionId}`, {
