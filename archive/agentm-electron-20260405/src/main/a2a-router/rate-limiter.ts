...
export class RateLimiter {
    ...
    constructor(name: string, config: Partial<RateLimitConfig> = {}) {
        ...
        // Register metrics
        metrics.gauge('a2a_rate_limiter_tokens', `Rate limiter tokens for ${name}`);
        metrics.gauge('a2a_rate_limiter_queue_size', `Rate limiter queue size for ${name}`);
        metrics.counter('a2a_rate_limiter_rejected', `Rate limiter rejections for ${name}`);
    }
    ...
    async tryExecute<T>(operation: () => Promise<T>): Promise<T> {
        if (!this.canProceed()) {
            this.rejected++;
            metrics.inc('a2a_rate_limiter_rejected', 1, { limiter: this.name });
    ...
    private enqueue(request: QueuedRequest<unknown>): void {
        ...
        metrics.set('a2a_rate_limiter_queue_size', this.queue.length, { limiter: this.name });
    }
