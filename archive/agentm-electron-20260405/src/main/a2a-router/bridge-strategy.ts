...
export class ReliabilityStrategy implements BridgeStrategy {
    ...
    updateMetrics(bridge: BridgeType, success: boolean, latency: number, cost: bigint): void {
        ...
        const alpha = 0.3;
        metric.successRate = success ? metric.successRate * (1 - alpha) + alpha : metric.successRate * (1 - alpha);
        metric.averageLatency = metric.averageLatency * (1 - alpha) + latency * alpha;
        metric.averageCost =
            (metric.averageCost * BigInt(100 - Math.floor(alpha * 100)) + cost * BigInt(Math.floor(alpha * 100))) /
            BigInt(100);
...
export class BridgeStrategyManager {
    ...
    async sendWithStrategy(message: A2AMessage, context: BridgeContext): Promise<A2AResult> {
