...
import type { AgentProfile, ProfileDraft, ProfileStatus } from '@/types';
import { useProStore } from '@/lib/store';

const PROFILE_STORAGE_KEY = 'agentm-pro:profiles:v1';
...
export function useProfile(owner: string | null): UseProfileResult {
    const profiles = useProStore((state) => state.profiles);
    const setProfiles = useProStore((state) => state.setProfiles);
    const setCurrentProfile = useProStore((state) => state.setCurrentProfile);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const stableOwner = useMemo(() => owner ?? 'demo-owner', [owner]);
    ...
    const refreshProfiles = useCallback(async () => {
        ...
        try {
            ...
            setProfiles(mine);
            setCurrentProfile(mine[0] ?? null);
        } catch (cause) {
            setError(cause instanceof Error ? cause.message : 'Failed to load profiles');
        } finally {
            setLoading(false);
        }
    }, [setCurrentProfile, setProfiles, stableOwner]);
