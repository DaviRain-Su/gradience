...
test.describe('AgentM Pro Metaplex A2A interactions', () => {
    ...
    test('creates delegation, sends A2A message, and settles token payment', async ({ page }) => {
        ...
        await expect(page.getByTestId('a2a-delegation-created')).toContainText('Generate collection metadata');
        ...
        await expect(page.getByTestId('a2a-chat-panel').getByTestId('a2a-chat-message-content').last()).toHaveText(
