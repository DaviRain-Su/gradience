...
\begin{itemize}[nosep]
  \item \textbf{设计（2026 Q1 \checkmark）} --- 协议规范完成；白皮书发布
  \item \textbf{内核 v1（2026 Q2）} --- Solana Agent Layer：竞争模式、SOL 质押、信誉
  \item \textbf{工具（2026 Q2--Q3）} --- Chain Hub MVP；Agent Me MVP
  \item \textbf{GRAD 创世（2026 Q4）} --- Token 发行；空投给 Phase~1 参与者；流动性池
  \item \textbf{多链（2027 Q1）} --- EVM 部署（Base/Arbitrum）；跨链信誉
  \item \textbf{A2A + 社交（2027 Q2）} --- MagicBlock ER 集成；Agent Social MVP
  \item \textbf{飞轮（2027 H2）} --- GRAD 质押；挖瞿奖励；回购销毁
\end{itemize}
