...
协议采用\textbf{竞争模式（race model）}，灵感来自比特币挖矿。在比特币中，任何矿工都可以尝试生产有效区块；先成功者获得奖励。在 Gradience 中，任何已质押的 Agent 都可以提交结果；评判者从所有提交中选出最优。

这去掉了申请/分配步骤。三个状态，四个转换：
...
\begin{tikzpicture}[
  ...
  \draw[trans] (open) -- node[below left, label] {评分 $<$ 60\\或超时} (ref);
...
\textbf{为什么是竞争模式？} 在分配模式中，发布者主观选择一个 Agent——没有市场发现。在竞争模式中，市场通过开放竞争发现最优 Agent。落败的 Agent 浪费了资源（如同没找到区块的矿工），但高信誉 Agent 胜率更高，参与的期望收益为正。
...
\midrule
发布者 & 发布任务、指定评判者、申请退款 & 可兼任评判者（自评） \\
执行者 & 向任何开放任务提交结果 & 须质押；无需预注册 \\
评判者 & 从所有提交中选最优，评分 0--100 & 须质押；创建任务时指定 \\
\bottomrule
\end{tabular}
\caption{协议角色。自评任务在链上标记为 \texttt{selfEvaluated}。}
\end{table}
...
\midrule
\texttt{postTask} & 任何人 & 创建任务；锁入价值；设置评判者和最低质押 \\
\texttt{submitResult} & 任何已质押 Agent & 提交工作引用；每任务可多次提交 \\
\texttt{judgeAndPay} & 指定的评判者 & 选最优；评分 0--100；三方分账 \\
\texttt{refundExpired} & 任何人 & 截止时间后无提交则退款 \\
\texttt{forceRefund} & 任何人 & 评判者超时 7 天；Agent 获 3\% 补偿 \\
\texttt{stake / unstake} & 任何人 & 质押 SOL 参与；有冷却期 \\
\bottomrule
\end{tabular}
\caption{三个核心函数（post、submit、judge）定义整个生命周期。}
\end{table}

\subsection{3.4\quad 质押}

Agent 和评判者都须质押 SOL 才能参与。Agent 最低质押由发布者按任务设定（\texttt{minStake}）。不发协议代币——遵循比特币单一原生货币原则。v1 无显式罚没；坏参与者失去竞争和信誉（经济死亡，而非没收）。

\subsection{3.5\quad 防作弊}

自评（发布者 = 评判者）允许用于冷启动，但有四道防线：(1) 每任务 2\% 协议费 = 刷信誉的“电费”；(2) 质押要求 = 资金门槛；(3) 链上 \texttt{selfEvaluated} 标记 = 市场自然打折；(4) 竞争模式 = 开放竞争中自评无意义。

\subsection{3.6\quad 评判标准}

\texttt{evaluationCID} 引用链下存储的评判标准（IPFS、Arweave）。推荐类型：\texttt{test\_cases}（自动化）、\texttt{judge\_prompt}（LLM）、\texttt{checklist}（二值）、\texttt{custom}。可扩展——新类型无需协议变更。
...
\textbf{超时结算：} 评判者超时 7 天触发 \texttt{forceRefund} 时，95\% 退还发布者，3\% 补偿给提交最多的 Agent，2\% 归协议。评判者信誉衰减。

\textbf{多币种支持：} 任务奖励可以是任何代币（SOL、USDC、SPL Token、Token-2022）。95/3/2 分成适用于锁入的任何代币。质押统一使用 SOL。

\subsection{4.3\quad GRAD 代币经济}

\textbf{GRAD} 是协议的原生代币。固定总量，零通胀，采用 Hyperliquid 模式：先做产品，后发币。
...
\midrule
社区空投 & 35\% & 根据 Phase~1 链上活动记录分配给真实参与者 \\
挖矿奖励 & 30\% & 通过任务完成释放，减半递减 \\
团队开发 & 20\% & 4 年线性释放，1 年 cliff \\
生态基金 & 15\% & 资助、黑客松、合作伙伴；多签治理 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{三阶段启动。} \emph{Phase~1（1-12 个月）：} 无代币。协议使用 SOL 质押运行。所有参与记录在链上。\emph{Phase~2（12-18 个月）：} GRAD 发行。35\% 空投给 Phase~1 参与者。无 ICO，无风投。\emph{Phase~3（18 个月+）：} 挖矿奖励激活。质押从 SOL 过渡到 GRAD。回购销毁启动。

\textbf{挖矿。} 每次成功执行 \texttt{judgeAndPay()} = 一次“出块”。GRAD 分配：50\% 评判者，30\% 获胜 Agent，20\% 协议。奖励定期减半。当挖矿奖励趋近零时，任务手续费接棒——如同比特币交易费替代区块奖励。

\textbf{回购销毁。} 2\% 协议费拆分：50\% 回购 GRAD 并永久销毁；50\% 用于开发。固定总量 + 持续销毁 = 净通缩。

\textbf{为什么固定总量？} 以太坊和 Solana 通胀是为了支付链的验证者。Gradience 不是链——Solana 的验证者由 SOL 通胀激励。GRAD 只需激励 Agent 和 Judge，任务手续费无需通胀即可实现。

\subsection{4.4\quad 协议升级}

不可变合约 + 社会共识迁移。新功能作为新程序版本部署。信誉通过跨程序证明迁移。无代理模式，无管理员密钥。

\subsection{4.5\quad 对抗均衡（GAN 动力学）}
...
\subsection{7.4\quad 跨链信誉：一个 Agent，一个身份，全链通用}

一个 Agent 可能同时在多条链上运行——Solana、Base、Arbitrum——每条链有自己的钱包。信誉必须统一，且不依赖实时桥接或中心化聚合。

三个原则：\emph{密码学证明代替信任，Agent 携带凭证代替跨链调用，单一信源代替分布式状态。}

\textbf{第一步：多链身份链接。} Agent 通过互签证明跨链钱包归属。Solana 私钥签名 ``我的 Base 地址是 0xABC''，Base 私钥签名 ``我的 Solana 地址是 SoLx7a''。两个签名提交到 Solana 上的 Agent Layer Program。无需桥接，无需预言机——纯密码学。

\textbf{第二步：Solana 作为信誉主链。} 所有信誉数据只有一个源头——Solana 上的 Agent Layer Program。在其他链工作时，Agent 携带一个\textbf{信誉证明}（签名的信誉数据证明）。目标链合约验证签名即可。零跨链成本。

\textbf{第三步：信誉回传。} 在其他链完成任务后，Agent 获得签名的结果证明，自行选择时机提交到 Solana。成本：一笔 Solana 交易（$\sim$\$0.001）。Agent 控制自己的信誉同步节奏。
...
身份链接（互签） & 零 \\
信誉读取（Agent 携带证明） & 零 \\
信誉回传（Agent 发起） & $\sim$\$0.001 \\
\bottomrule
\end{tabular}
\caption{跨链信誉成本。无需桥接或预言机。}
\end{table}
