...
describe('AgentArenaEVM', function () {
    async function deployFixture() {
        const [deployer, poster, judge, agentA, agentB, treasury, attacker] = await ethers.getSigners();
    ...
    it('post_task creates an open task with reward locked', async function () {
        ...
        await expect(
            contract.connect(poster).postTask('cid://eval', deadline, judgeDeadline, judge.address, 1, 0n, false, {
                value: reward,
            }),
    ...
    it('apply_for_task + submit_result + judge_and_pay completes payout path', async function () {
        ...
        expect(winnerBalanceAfterJudge - winnerBalanceBefore).to.equal((reward * 9500n) / 10000n);
        ...
        await expect(() => contract.connect(agentA).claimStake(1n)).to.changeEtherBalance(
            agentA,
            (minStake * 11n) / 10n,
        );
        await expect(() => contract.connect(agentB).claimStake(1n)).to.changeEtherBalance(
            agentB,
            (minStake * 11n) / 10n,
        );
    ...
    it('supports ERC20 reward and stake lifecycle', async function () {
        const { contract, poster, judge, agentA, agentB, attacker, treasury, token } = await deployErc20Fixture();
        const reward = ethers.parseEther('1');
        const minStake = ethers.parseEther('0.1');
        const now = await currentTime();
        const deadline = now + 3600n;
    ...
    it('judge_and_pay rejects non-judge signer', async function () {
        ...
        await contract.connect(poster).postTask('cid://eval', deadline, judgeDeadline, judge.address, 1, 0n, false, {
            value: reward,
        });
    ...
    it('submit_result rejects overwrite and judge cannot apply', async function () {
        ...
        await contract.connect(agentA).applyForTask(1n, { value: (minStake * 11n) / 10n });
        await contract.connect(agentA).submitResult(1n, 'cid://result-a', 'cid://trace-a');
        await expect(contract.connect(agentA).submitResult(1n, 'cid://result-b', 'cid://trace-b'))
    ...
    describe('Dispute', function () {
        ...
        it('resolver can uphold dispute and redistribute', async function () {
            ...
            const tx = await contract.connect(deployer).resolveDispute(1n, 1, agentA.address, 80); // DisputeOutcome.Uphold = 1
            await expect(tx)
    ...
    describe('JudgeRegistry Integration (GRA-179)', function () {
        ...
        it('auto-assigns judge from registry when judge is zero address', async function () {
            ...
            await expect(
                contract
                    .connect(poster)
                    .postTask('cid://eval', deadline, judgeDeadline, ethers.ZeroAddress, 2, 0n, false, {
                        value: reward,
                    }),
            )
                .to.emit(contract, 'TaskCreated')
        ...
        it('reassign_judge slashes inactive judge and picks new one', async function () {
            ...
            await expect(contract.reassignJudge(1n)).to.be.revertedWithCustomError(contract, 'ReassignWindowNotOpen');
    ...
    describe('Reputation Integration (GRA-243 / GRA-248)', function () {
        async function deployWithReputationFixture() {
            ...
            const initData = feedFactory.interface.encodeFunctionData('initialize', [
        ...
        it('applies with base stake when reputationFeed is not set', async function () {
            ...
            await contract
                .connect(poster)
                .postTask('cid://eval', now + 3600n, now + 7200n, poster.address, 2, minStake, false, {
                    value: reward,
                });
            await expect(contract.connect(agentA).applyForTask(1n, { value: (minStake * 11n) / 10n })).to.not.be
                .reverted;
        });
        ...
        it('requires 2x stake when reputationFeed is set but agent has no record', async function () {
            ...
            await expect(
                contract.connect(agentA).applyForTask(1n, { value: (minStake * 11n) / 10n }),
            ).to.be.revertedWithCustomError(contract, 'InvalidStakeAmount');
            await expect(contract.connect(agentA).applyForTask(1n, { value: (minStake * 22n) / 10n })).to.not.be
                .reverted;
        });
        ...
        it('applies with base stake when reputationFeed has agent record', async function () {
            ...
            const sig = signReputationUpdate(
                oracleWallet,
                agentA.address,
                ethers.ZeroHash,
                75,
