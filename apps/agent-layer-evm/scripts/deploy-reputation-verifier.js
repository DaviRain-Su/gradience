...
async function main() {
    ...
    const signerPubkey = requireHex32(
        process.env.ED25519_SIGNER_PUBKEY_HEX,
        "ED25519_SIGNER_PUBKEY_HEX",
    );
    const maxAttestationAge = BigInt(
        process.env.MAX_ATTESTATION_AGE_SECONDS || String(7 * 24 * 60 * 60),
    );
