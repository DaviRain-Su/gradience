...
const { createRelayServer, signHttpPayload, toVerifierPayload } = require('./reputation-relay-server.js');
...
test('relay server supports ERC-8004 registration and feedback routes', async () => {
    ...
    try {
        ...
        const registerResponse = await postSigned('/relay/erc8004/register-identity', registrationPayload);
        ...
        const registerAgainResponse = await postSigned('/relay/erc8004/register-identity', registrationPayload);
        ...
        const feedbackResponse = await postSigned('/relay/erc8004/give-feedback', {
            agentPubkey: agent,
            value: 88,
            valueDecimals: 0,
            tag1: 'taskScore',
