...
contract AgentArenaEVM is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuard {
    ...
    function judgeAndPay(uint256 task_id, address winner, uint8 score) external nonReentrant {
        Task storage task = _loadOpenTask(task_id);
    ...
    function submitJudgement(uint256 task_id, address winner, uint8 score) external {
        Task storage task = tasks[task_id];
        if (task.poster == address(0)) revert TaskNotFound(task_id);
        if (task.state != TaskState.Open) revert TaskNotOpen(task_id);
        if (block.timestamp > task.judgeDeadline) revert JudgeDeadlinePassed(task_id);
    ...
    function settleWithQuorum(uint256 task_id) external nonReentrant {
        Task storage task = tasks[task_id];
        if (task.poster == address(0)) revert TaskNotFound(task_id);
        if (task.state != TaskState.Open) revert TaskNotOpen(task_id);
    ...
    function disputeTask(uint256 task_id, bytes32 reason_hash) external payable nonReentrant {
        Task storage task = tasks[task_id];
        if (task.poster == address(0)) revert TaskNotFound(task_id);
    ...
    function claimExpired(uint256 task_id) external nonReentrant {
        Task storage task = _loadOpenTask(task_id);
        if (block.timestamp <= task.judgeDeadline) revert JudgeDeadlineNotReached(task_id);
    ...
    function forceRefund(uint256 task_id) external nonReentrant {
        Task storage task = _loadOpenTask(task_id);
        if (block.timestamp <= task.judgeDeadline) revert JudgeDeadlineNotReached(task_id);
    ...
    function reassignJudge(uint256 task_id) external nonReentrant {
        Task storage task = tasks[task_id];
        if (task.poster == address(0)) revert TaskNotFound(task_id);
        if (task.judgeMode != uint8(JudgeMode.DESIGNATED)) revert InvalidJudgeMode(task_id);
    ...
    function cancelTask(uint256 task_id) external nonReentrant {
        Task storage task = _loadOpenTask(task_id);
    ...
    function claimStake(uint256 task_id) external nonReentrant {
        Task storage task = tasks[task_id];
        if (task.poster == address(0)) revert TaskNotFound(task_id);
        if (task.state == TaskState.Open) revert TaskNotOpen(task_id);
    ...
    function getTaskJudgePool(uint256 task_id) external view returns (address[] memory) {
        return taskJudgePool[task_id];
    }

    function getSubmission(uint256 task_id, address agent) external view returns (Submission memory) {
        return _submissions[task_id][agent];
    }

    function getApplicants(uint256 task_id) external view returns (address[] memory) {
