...
export class GradienceOracleClient {
    ...
    async getReputationProof(evmAddress: string, chainId: string, solanaPubkey?: string): Promise<ReputationResponse> {
        // 调用 Oracle 服务获取签名证明
    }

    async syncReputationToChain(chainId: string, response: ReputationResponse): Promise<string> {
        // 用户自己提交上链（需钱包签名 gas）
...
_文档状态: Phase 3 Technical Spec — Ready for Phase 4 Task Breakdown_
