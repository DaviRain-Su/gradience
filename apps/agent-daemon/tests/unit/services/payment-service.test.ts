...
import { OWSWalletManager } from '../../../src/wallet/ows-wallet-manager.js';
import type { A2ARouter } from '../../../src/a2a-router/router.js';
import type { EvaluatorRuntime, EvaluationResult } from '../../../src/evaluator/index.js';
...
describe('PaymentService', () => {
  ...
  let mockEvaluator: EvaluatorRuntime;
  let mockBridgeManager: BridgeManager;
  let paymentService: PaymentService;
  let tmpDir: string;

  function createMockEvaluator(): EvaluatorRuntime {
    let storedHandler: ((result: EvaluationResult) => void) | null = null;

    const evaluator = {
      submit: vi.fn().mockResolvedValue('eval-test-123'),
      on: vi.fn().mockImplementation((event: string, handler: any) => {
        if (event === 'completed') {
          storedHandler = handler;
          // Auto-resolve evaluation after a microtask so evaluateAndSettle completes
          Promise.resolve().then(() => {
            handler({
              ...
              executionLog: {
                sandboxType: 'git_worktree' as const,
                steps: [],
                stdout: '',
                stderr: '',
              },
      ...
      off: vi.fn().mockImplementation((event: string, handler: any) => {
        if (event === 'completed' && storedHandler === handler) {
          storedHandler = null;
        }
      }),
    } as unknown as EvaluatorRuntime;
  ...
  beforeEach(async () => {
    ...
    mockBridgeManager = createMockBridgeManager();
  ...
  describe('payment request', () => {
    ...
    it('should reject payment exceeding policy limits', async () => {
      ...
      ).rejects.toThrow('exceeds max transaction');
    });
  });
  ...
  describe('payment acceptance', () => {
    ...
    it('should reject acceptance by wrong agent', async () => {
      ...
      await expect(
        paymentService.acceptPayment(session.paymentId, 'wrong-agent')
      ).rejects.toThrow('Agent is not the designated payee');
    });

    it('should reject acceptance of non-existent payment', async () => {
      await expect(
        paymentService.acceptPayment('non-existent', 'payee-agent')
  ...
  describe('service completion', () => {
    it('should mark service complete', async () => {
      ...
      const completed = await paymentService.markServiceComplete(
        session.paymentId,
        'payee-agent'
      );
  ...
  describe('disputes', () => {
    it('should raise dispute', async () => {
      ...
      const disputed = await paymentService.raiseDispute(
        session.paymentId,
        'payer-agent',
        {
          initiator: 'payer',
