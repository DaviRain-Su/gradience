    ...
    const mockVerifyAPI = vi.fn().mockResolvedValue({
        ...
        details: [
            ...
            },
            {
                ...
                metadata: { status: 201, duration: 100 },
            },
        ],
        durationMs: 500,
    });
...
describe('Evaluator Integration', () => {
    ...
    describe('Judge Registry Integration', () => {
        ...
        it('should handle multiple evaluations concurrently', async () => {
            ...
            const results = await Promise.all([codeJudge.evaluate(tasks[0]), contentJudge.evaluate(tasks[1])]);
                    ...
                    submission:
                        type === 'url'
                            ? {
                                  type: 'url',
                                  source: 'https://example.com',
                                  metadata: { endpoints: [{ method: 'GET', path: '/health', expectedStatus: 200 }] },
                              }
