...
export class TaskService extends TaskQueue {
  ...
  private initEvaluatorRuntime(): void {
    try {
      ...
      const llmClient = unifiedConfig
        ? createLLMClientFromConfig(unifiedConfig)
        : null;
      ...
      const scoringProvider = provider === 'claude' ? 'anthropic' :
        provider === 'moonshot' ? 'openai' : provider;
      ...
      logger.info(
        { provider, model, unified: !!unifiedConfig },
        'Evaluator runtime initialized'
      );
    } catch (error) {
  ...
  private getDefaultCategories(evalType: EvaluationType): Array<{
    ...
    const categories: Record<EvaluationType, Array<{
      name: string;
      weight: number;
      description: string;
      criteria: string[];
      ...
      code: [
        { name: 'Functionality', weight: 0.4, description: 'Code works as expected', criteria: ['Correctness', 'Test coverage'] },
        { name: 'Quality', weight: 0.3, description: 'Code quality and style', criteria: ['Readability', 'Maintainability'] },
        { name: 'Security', weight: 0.2, description: 'Security best practices', criteria: ['Input validation', 'Error handling'] },
        { name: 'Documentation', weight: 0.1, description: 'Documentation quality', criteria: ['Comments', 'README'] },
      ],
      ui: [
        { name: 'Visual', weight: 0.3, description: 'Visual design quality', criteria: ['Layout', 'Color scheme'] },
        { name: 'Accessibility', weight: 0.25, description: 'Accessibility compliance', criteria: ['WCAG', 'Keyboard navigation'] },
        { name: 'Responsive', weight: 0.25, description: 'Responsive design', criteria: ['Mobile', 'Tablet'] },
        { name: 'Performance', weight: 0.2, description: 'UI performance', criteria: ['Load time', 'Interactivity'] },
      ],
      api: [
        { name: 'Correctness', weight: 0.4, description: 'API correctness', criteria: ['Contract compliance', 'Status codes'] },
        { name: 'Performance', weight: 0.3, description: 'API performance', criteria: ['Latency', 'Throughput'] },
        { name: 'Documentation', weight: 0.2, description: 'API documentation', criteria: ['OpenAPI', 'Examples'] },
        { name: 'Security', weight: 0.1, description: 'API security', criteria: ['Auth', 'Rate limiting'] },
      ],
      content: [
        { name: 'Accuracy', weight: 0.3, description: 'Content accuracy', criteria: ['Factual correctness', 'Sources'] },
        { name: 'Clarity', weight: 0.25, description: 'Content clarity', criteria: ['Structure', 'Language'] },
        { name: 'Completeness', weight: 0.25, description: 'Content completeness', criteria: ['Coverage', 'Depth'] },
        { name: 'Originality', weight: 0.2, description: 'Content originality', criteria: ['Uniqueness', 'Insights'] },
      ],
      composite: [
