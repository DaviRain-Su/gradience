...
export async function runX402EvmSelfTest(
  ...
  if (!privateKey) {
    return { success: false, error: 'x402EvmPrivateKey not configured (set AGENTD_X402_EVM_PRIVATE_KEY or PRIVATE_KEY)' };
  }
  ...
  try {
    ...
    if (decodedEvent.actualAmount !== actualAmount) {
      throw new Error(
        `Settled amount mismatch: expected ${actualAmount}, got ${decodedEvent.actualAmount}`
      );
    }
    if (decodedEvent.refunded !== maxAmount - actualAmount) {
      throw new Error(
        `Refund mismatch: expected ${maxAmount - actualAmount}, got ${decodedEvent.refunded}`
      );
    }
