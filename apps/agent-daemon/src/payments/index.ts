...
export class PaymentManager {
    ...
    constructor(config: Partial<PaymentsConfig>, keyManager: KeyManager, transactionManager: ITransactionManager) {
        this.config = {
    ...
    async fundMPPEscrow(paymentId: string): Promise<{ txSignature: string }> {
        ...
        if (payment.payer !== daemonPubkey) {
            throw new DaemonError(ErrorCodes.PAYMENT_UNAUTHORIZED, 'Only the payer can fund this escrow', 403);
        }
     ...
     */
    async createX402Authorization(requirements: X402PaymentRequirements): Promise<X402Authorization> {
        this.ensureX402Initialized();
     ...
     */
    async processX402Authorization(authorization: X402Authorization): Promise<{ txSignature: string; status: string }> {
        this.ensureX402Initialized();
    ...
    async executeTransfer(params: {
        ...
        try {
            ...
            if (params.token && params.token !== SystemProgram.programId.toBase58()) {
                ...
                throw new DaemonError(ErrorCodes.NOT_IMPLEMENTED, 'SPL token transfers not yet implemented', 400);
            } else {
                // SOL transfer
                transaction.add(
                    SystemProgram.transfer({
            ...
            logger.info({ txSignature, to: params.to, amount: params.amount.toString() }, 'Direct transfer executed');
    ...
    private ensureMPPInitialized(): void {
        if (!this.initialized) {
            throw new DaemonError(ErrorCodes.INVALID_REQUEST, 'PaymentManager not initialized', 500);
        }
        if (!this.config.mppEnabled || !this.mppPaymentManager) {
            throw new DaemonError(ErrorCodes.INVALID_REQUEST, 'MPP payments not enabled', 400);
        }
    }
    ...
    private ensureX402Initialized(): void {
        ...
        if (!this.config.x402Enabled || !this.x402Handler) {
            throw new DaemonError(ErrorCodes.INVALID_REQUEST, 'X402 payments not enabled', 400);
        }
    }
