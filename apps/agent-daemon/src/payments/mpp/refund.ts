...
export class MPPRefund {
  ...
  async releaseFunds(
    payment: MPPPayment,
    signer: Signer
  ): Promise<{ txSignatures: string[]; newStatus: MPPStatus }> {
    ...
    for (const participant of payment.participants) {
      ...
      try {
        ...
        logger.info(
          { paymentId: payment.paymentId, participant: participant.address, amount: releaseAmount.toString() },
  ...
  async refund(
    ...
    logger.info(
      { paymentId: payment.paymentId, amount: refundAmount.toString() },
      'Payment refunded'
    );
