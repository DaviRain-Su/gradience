...
export class MPPPaymentManager {
     ...
     */
    async fundEscrow(paymentId: string, escrowAddress: string, signer: Signer): Promise<{ txSignature: string }> {
        ...
        if (payment.status !== 'pending_funding') {
            throw new DaemonError(ErrorCodes.PAYMENT_INVALID_STATE, `Cannot fund in state: ${payment.status}`, 400);
        }
