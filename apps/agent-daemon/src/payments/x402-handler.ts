...
export interface X402Authorization {
  ...
  /** Authorization type */
  type: 'solana_transaction' | 'evm_transaction' | 'signature';
  /** Timestamp */
  timestamp: number;
...
export class X402Handler extends EventEmitter {
  ...
  createPaymentRequirements(params: {
    ...
    if (!tokenInfo) {
      throw new DaemonError(
        ErrorCodes.PAYMENT_TOKEN_NOT_SUPPORTED,
        `Token not supported: ${params.token}`,
        400
      );
    }
  ...
  async createAuthorization(
    ...
    logger.info(
      { paymentId: requirements.paymentId, payer: auth.payer },
      'x402 authorization created'
    );
  ...
  async processAuthorization(
    ...
    try {
      ...
      logger.info(
        { paymentId: authorization.paymentId, txSignature },
      ...
      logger.error(
        { error, paymentId: authorization.paymentId },
        'x402 payment processing failed'
      );
  ...
  private findSessionByPaymentId(paymentId: string): X402Session | undefined {
    return Array.from(this.sessions.values()).find(
      s => s.requirements.paymentId === paymentId
    );
  }
