...
export class RevenueSharingEngine {
     ...
     */
    recordTaskDistribution(taskInfo: TaskSettlementInfo): { distributionId: string; calculation: RevenueCalculation } {
     ...
     */
    async settleDistribution(distributionId: string, signer: Signer): Promise<DistributionResult> {
        const record = this.store.getById(distributionId);
        if (!record) {
            throw new DaemonError(ErrorCodes.NOT_FOUND, `Distribution ${distributionId} not found`);
        }
        ...
        if (record.status === 'confirmed') {
            ...
            return {
                ...
                breakdown: {
                    ...
                    protocol: {
                        address: this.config.protocolTreasury.toBase58(),
                        amount: BigInt(record.protocolAmount),
                    },
                },
                status: 'confirmed',
            };
        ...
        try {
            ...
            const isNativeSOL =
                record.tokenMint === PublicKey.default.toBase58() ||
            ...
            throw new DaemonError(ErrorCodes.TRANSACTION_FAILED, `Settlement failed: ${errorMsg}`);
        }
    }
