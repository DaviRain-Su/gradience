...
export class RevenueStore {
    ...
    getStats(): RevenueStats {
        ...
        const pendingStmt = this.db.prepare(
            "SELECT COUNT(*) as count FROM revenue_distributions WHERE status = 'pending'",
        );
        const { count: pendingCount } = pendingStmt.get() as { count: number };

        const confirmedStmt = this.db.prepare(
            "SELECT COUNT(*) as count FROM revenue_distributions WHERE status = 'confirmed'",
        );
        const { count: confirmedCount } = confirmedStmt.get() as { count: number };

        const failedStmt = this.db.prepare(
            "SELECT COUNT(*) as count FROM revenue_distributions WHERE status = 'failed'",
        );
