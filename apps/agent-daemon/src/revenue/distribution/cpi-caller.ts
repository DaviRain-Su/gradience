...
export class CPICaller {
    ...
    async call(config: DistributionConfig, request: DistributionRequest, signer: Signer): Promise<DistributionResult> {
...
import type { DistributionRequest, DistributionResult, DistributionConfig } from './types.js';
...
export async function distribute(
    ...
    if (request.escrowAccount.equals(SystemProgram.programId)) {
        throw new DaemonError(
            ErrorCodes.INVALID_ESCROW,
            'Invalid escrow account: system program cannot be used as escrow',
        );
    }
    ...
    try {
        ...
        throw new DaemonError(ErrorCodes.TRANSACTION_FAILED, error instanceof Error ? error.message : 'Unknown error');
    }
}
...
export async function distributeTokens(
    ...
    try {
        ...
        // Transfer to agent (95%)
        transaction.add(
            createTransferInstruction(escrowATA, agentATA, request.escrowAuthority, breakdown.agent.amount),
        );

        // Transfer to judge (3%)
        transaction.add(
            createTransferInstruction(escrowATA, judgeATA, request.escrowAuthority, breakdown.judge.amount),
        );

        // Transfer to protocol (2%)
        transaction.add(
            createTransferInstruction(escrowATA, protocolATA, request.escrowAuthority, breakdown.protocol.amount),
        );
