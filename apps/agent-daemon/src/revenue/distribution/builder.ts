...
import { DistributionConfig, DistributionRequest, TokenAccounts, DistributionBreakdown } from './types.js';
...
export class DistributionBuilder {
    ...
    constructor(options: { config: DistributionConfig }) {
        ...
        const total =
            options.config.percentages.agent + options.config.percentages.judge + options.config.percentages.protocol;
        if (total !== 10000) {
     ...
     */
    calculateBreakdown(totalAmount: bigint, agentAddress: PublicKey, judgeAddress: PublicKey): DistributionBreakdown {
...
export function createDistributionBuilder(options: DistributionBuilderFactoryOptions): DistributionBuilder {
