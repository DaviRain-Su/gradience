...
export class OpenWalletManager {
    ...
    private isPhantomAvailable(): boolean {
        ...
        return typeof globalThis !== 'undefined' && (globalThis as any).phantom?.solana?.isPhantom === true;
    }
    ...
    private isSolflareAvailable(): boolean {
        return typeof globalThis !== 'undefined' && (globalThis as any).solflare?.isSolflare === true;
    }
    ...
    private isBackpackAvailable(): boolean {
        return typeof globalThis !== 'undefined' && (globalThis as any).backpack?.isBackpack === true;
    }
    ...
    async connect(walletName?: string): Promise<void> {
        ...
        if (walletName) {
            ...
            if (!wallet) {
                throw new DaemonError(ErrorCodes.WALLET_NOT_FOUND, `Wallet ${walletName} not found`, 404);
            }
        } else {
