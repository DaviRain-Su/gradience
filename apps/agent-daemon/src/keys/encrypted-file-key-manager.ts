...
export class EncryptedFileKeyManager {
        ...
        if (existsSync(this.keyPath)) {
            ...
            if (this.isEncrypted(data)) {
                ...
                try {
                    ...
                    throw new DaemonError(ErrorCodes.KEY_INVALID, 'Failed to load keyfile: invalid format', 400);
                }
            }
        } else {
    ...
    private async saveEncrypted(password: string): Promise<void> {
        ...
        const data = Buffer.concat([Buffer.from([ENCRYPTION_VERSION]), salt, iv, authTag, ciphertext]);
