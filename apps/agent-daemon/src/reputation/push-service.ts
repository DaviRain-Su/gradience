...
export class ReputationPushService {
    ...
    private async pushToERC8004(
        ...
        try {
            ...
            const { txHash } = await this.config.erc8004Client.giveFeedback({
                ...
                feedbackURI: `data:application/json;base64,${Buffer.from(JSON.stringify(score)).toString('base64')}`,
    ...
    async batchPush(agentAddresses: string[]): Promise<{
        ...
        logger.info({ success: success.length, failed: failed.length }, 'Batch push completed');
    ...
    getSyncStatus(agentAddress: string): SyncStatus {
        return (
            this.syncStatus.get(agentAddress) || {
                agentAddress,
                solanaSynced: false,
    ...
    private startBatchJob(): void {
        ...
        logger.info({ intervalMs: this.config.batchIntervalMs }, 'Batch push job started');
    }
...
// Factory
export function createReputationPushService(config: PushServiceConfig): ReputationPushService {
