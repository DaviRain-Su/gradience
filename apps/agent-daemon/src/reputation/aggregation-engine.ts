...
export class ReputationAggregationEngine {
    ...
    constructor(config: Partial<ReputationCalculationConfig> = {}) {
        ...
        if (Math.abs(totalWeight - 1.0) > 0.001) {
            logger.warn({ totalWeight }, 'Reputation weights do not sum to 1.0, normalizing');
    ...
    calculateReputation(activity: AgentActivity): ReputationScore {
        ...
        const disputeRate = activity.attemptedTasks > 0 ? activity.disputeCount / activity.attemptedTasks : 0;
     ...
     */
    private calculateQualityScore(activity: AgentActivity, flags: string[]): number {
    ...
    private detectAnomalies(activity: AgentActivity, flags: string[]): void {
        ...
        if (
            activity.avgRating > this.config.suspiciousRatingThreshold &&
        ...
        if (activity.completedTasks > 50 && activity.dailyActivity.length < 7) {
    ...
    calculateBatch(activities: AgentActivity[]): Map<string, ReputationScore> {
        ...
        logger.info({ count: activities.length }, 'Batch reputation calculation completed');
