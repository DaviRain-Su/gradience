...
export function registerMagicBlockRoutes(app: FastifyInstance, bridgeManager?: BridgeManager): void {
    ...
    app.post<{
        ...
        try {
            ...
            const ix = selector.buildGradienceRequestRandomnessIx(taskId, Number(numericTaskId), payerPubkey);
    ...
    logger.info(
        'MagicBlock API routes registered: /api/v1/magicblock/session/*, /api/v1/magicblock/judge-per, /api/v1/magicblock/request-vrf',
    );
}
