...
export function registerEvaluatorRoutes(
  ...
  const runtime = evaluatorRuntime ?? new EvaluatorRuntime({
    defaultBudget: {
      maxCostUsd: 1.0,
      maxTimeSeconds: 300,
      maxMemoryMb: 512,
    ...
    try {
      ...
      const evalTask: EvaluationTask = {
        ...
        criteria: {
          ...
          requiredChecks: (criteria?.requiredChecks as EvaluationTask['criteria']['requiredChecks']) ?? ['compiles'],
        },
        budget: {
  ...
  app.post<{ Params: { id: string }; Body: ManualJudgeBody }>('/api/v1/evaluations/:id/judge', async (request, reply) => {
    try {
      const { id } = request.params;
