...
import { validateCreateTaskRequest, generateTaskId, generateSubtaskId } from '../../shared/coordinator-types.js';
...
export function registerCoordinatorRoutes(app: FastifyInstance): void {
    ...
    app.post<{
        ...
        try {
            ...
            const allCompleted = task.subtasks.every((s) => s.status === 'approved' || s.status === 'rejected');
