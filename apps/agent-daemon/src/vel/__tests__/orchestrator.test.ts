...
import type { TeeExecutionEngine, VelOrchestratorConfig, TeeExecutionResult, AttestationBundle, VerificationReport } from '../types';
...
describe('VelOrchestrator', () => {
  function createMockEngine(result: Partial<TeeExecutionResult> = {}): TeeExecutionEngine {
    return {
      execute: vi.fn().mockResolvedValue({
        ...
        attestationReport: Buffer.from(JSON.stringify({ pcr0: 'mock-pcr-allowed', userDataHash: 'resulthashloghash', signerIdentity: 'mock' })).toString('base64'),
        executedAt: Date.now(),
        ...result,
  ...
  it('H6: runAndSettle returns tx signature through mock bridge', async () => {
    ...
    const verifier = { verify: vi.fn().mockResolvedValue({ valid: true, pcrValues: { pcr0: 'mock-pcr-allowed' } } as VerificationReport) };
    ...
    expect(config.bridge.judgeAndPay).toHaveBeenCalledWith(
      expect.objectContaining({ taskId: 1, winner: 'So11111111111111111111111111111111111111112', score: 100, reasonRef: expect.stringContaining('vel:gramine-local:') })
    );
  });
  ...
  it('throws when attestation verification fails', async () => {
    const engine = createMockEngine();
    const verifier = { verify: vi.fn().mockResolvedValue({ valid: false, reason: 'PC tampered' } as VerificationReport) };
    const config = createConfig();
    const orchestrator = new DefaultVelOrchestrator(engine, verifier as any, config);
