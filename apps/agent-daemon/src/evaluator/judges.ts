...
import type { EvaluationTask, EvaluationResult, EvaluationCriteria, CategoryScore, CheckResult } from './runtime.js';
...
export abstract class BaseJudge extends EventEmitter {
    ...
    protected calculateOverallScore(scores: CategoryScore[]): number {
        ...
        const weightedScore = scores.reduce((sum, s) => sum + (s.score / s.maxScore) * s.weight * 100, 0);
        ...
        if (this.llmClient && isLLMAvailable()) {
            try {
                ...
                checks.push({
                    ...
                    passed: true,
                    score: 100,
                    details: 'LLM evaluation successful',
                    durationMs: 0,
                });
        ...
        checks.push(
            {
                type: 'compiles',
                passed: codeLength > 0,
                score: codeLength > 0 ? 100 : 0,
                details: 'Code submission present',
                durationMs: 0,
            },
            {
                type: 'tests_pass',
                passed: hasTests,
                score: hasTests ? 100 : 50,
