...
import type { EvaluationTask, EvaluationResult, CategoryScore, CheckResult } from './runtime.js';
...
export class ExternalEvaluatorClient extends EventEmitter {
     ...
     */
    async evaluate(request: ExternalEvaluationRequest): Promise<ExternalEvaluationResult> {
    ...
    handleWebhook(payload: unknown, signature: string): ExternalEvaluationResult | null {
        ...
        try {
            ...
            logger.info({ evaluationId: result.evaluationId }, 'Received webhook from external evaluator');
...
export class LLMEvaluator extends EventEmitter {
     ...
     */
    async evaluateContent(params: { content: string; criteria: string[]; context?: string }): Promise<{
        score: number;
        feedback: string[];
        analysis: string;
    }> {
     ...
     */
    private buildEvaluationPrompt(params: { content: string; criteria: string[]; context?: string }): string {
        return `
...
export function createExternalEvaluator(config: ExternalEvaluatorConfig): ExternalEvaluatorClient {
    return new ExternalEvaluatorClient(config);
}

export function createLLMEvaluator(config: LLMEvaluationConfig): LLMEvaluator {
