...
export class ChainHubIntegration extends EventEmitter {
  ...
  async settleTask(request: TaskSettlementRequest): Promise<SettlementResult> {
    ...
    try {
      ...
      logger.info(
        { settlementId, txSignature: result.txSignature },
        'Chain Hub settlement completed'
      );
  ...
  async batchSyncReputations(agentAddresses: string[]): Promise<{
    ...
    for (let i = 0; i < agentAddresses.length; i += batchSize) {
      ...
      const results = await Promise.allSettled(
        batch.map((address) => this.syncReputation(address))
      );
  ...
  getStats(): {
    ...
    const lastSync = Array.from(this.reputationCache.values()).reduce(
      (latest, current) =>
        current.lastSyncAt > latest ? current.lastSyncAt : latest,
      0
    );
...
export function createChainHubIntegration(
  config: ChainHubIntegrationConfig
): ChainHubIntegration {
