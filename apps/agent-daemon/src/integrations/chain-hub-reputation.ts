...
export class ChainHubReputationClient {
  ...
  async getReputation(agentAddress: string): Promise<ReputationRecord | null> {
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/api/agents/${encodeURIComponent(agentAddress)}/reputation`
      );
  ...
  async getReputationsByMaster(
    ...
    try {
      ...
      logger.debug(
        { masterWallet, count: results.length },
        'Fetched reputations by master from Chain Hub'
      );
      return results;
    } catch (error) {
  ...
  async getAggregateReputation(masterWallet: string): Promise<AggregateReputation | null> {
    ...
    const totalWeight = agents.reduce(
      (sum, a) => sum + Math.max(a.reputation.completedTasks, 1),
      0
    );
