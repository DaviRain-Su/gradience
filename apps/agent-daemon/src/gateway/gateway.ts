...
export interface ArenaTaskClient {
  post(params: { taskId: bigint; evalRef: string; deadline: bigint; judgeDeadline: bigint; judgeMode: number; judge: string; category: number; minStake: bigint; reward: bigint }): Promise<string>;
  apply(taskId: bigint): Promise<string>;
  submit(taskId: bigint, params: { resultRef: string; traceRef: string; runtimeEnv: Record<string, unknown> }): Promise<string>;
    ...
    while (true) {
      if (current.status === 'PENDING' || current.status === 'TASK_CREATING') {
        if (current.status === 'PENDING') {
          ...
          this.store.update(current.purchaseId, { status: taskCreating, updatedAt: new Date().toISOString() });
