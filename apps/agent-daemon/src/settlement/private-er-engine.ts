...
import { DefaultTeeExecutionEngine, TeeProviderFactory } from '../vel/tee-execution-engine.js';
import { DefaultVelOrchestrator } from '../vel/orchestrator.js';
import { AttestationVerifier } from '../vel/attestation-verifier.js';
import { TeeExecutionRequest, WorkflowDefinition, TeeExecutionResult, AttestationBundle } from '../vel/types.js';
...
export interface PrivateEREngineConfig {
    ...
    bridge: {
        judgeAndPay(params: { taskId: number; winner: string; score: number; reasonRef: string }): Promise<string>;
    };
 ...
 * inside the mock TEE.
 */
function buildJudgeWorkflowDefinition(submissions: SealedSubmission[], criteria: unknown): WorkflowDefinition {
    return {
        version: '1.0',
        name: 'private-judge',
        steps: [
...
export class PrivateEREngine {
    ...
    constructor(config: PrivateEREngineConfig) {
        ...
        this.orchestrator = new DefaultVelOrchestrator(this.engine, new AttestationVerifier(provider), {
    ...
    async initialize(): Promise<void> {
        ...
        const enclavePath = this.config.enclaveScriptPath ?? resolve(__dirname, '../../scripts/mock-per-enclave.mjs');
     ...
     */
    async judgeInPrivateSession(request: PrivateERJudgeRequest): Promise<PrivateERJudgeResult> {
     ...
     */
    async judgeAndSettle(request: PrivateERJudgeRequest): Promise<{ txSig: string; result: PrivateERJudgeResult }> {
