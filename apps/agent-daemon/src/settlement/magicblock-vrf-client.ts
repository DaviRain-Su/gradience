...
export const MAGICBLOCK_VRF_PROGRAM_ID = new PublicKey('Vrf1RNUjXmQGjmQrQLvJHs9SNkvDJEsRVFPkfSQUwGz');
...
function borshSerializeAccountMetas(metas: SerializableAccountMeta[]): Buffer {
...
function readQueueItem(data: Buffer, offset: number): QueueItem {
    ...
    return {
        ...
        callbackProgramId: new Uint8Array(data.buffer, data.byteOffset + offset + 40, 32),
...
export function* iterateQueueItems(data: Buffer): Generator<QueueItem, void, unknown> {
    ...
    while (cursor + QUEUE_ITEM_SIZE <= end) {
        ...
        const itemEnd = cursor + QUEUE_ITEM_SIZE + item.callbackDiscriminatorLen + metasBytes + item.argsLen;
...
export function findQueueItemById(data: Buffer, id: Uint8Array): QueueItem | undefined {
