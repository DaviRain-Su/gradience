...
export class VRFJudgeSelector {
    ...
    constructor(options: VRFJudgeSelectorOptions = {}) {
        ...
        this.connection = new Connection(options.rpcEndpoint ?? config.solanaRpcUrl, 'confirmed');
        this.vrfProgramId = new PublicKey(options.vrfProgramId ?? MAGICBLOCK_VRF_PROGRAM_ID.toBase58());
    ...
    buildRequestRandomnessIx(
        ...
        const programIdentity = PublicKey.findProgramAddressSync([Buffer.from('identity')], this.vrfProgramId)[0];
...
export class JudgeRotationManager {
    ...
    async selectNextJudge(
        ...
        const eligible = candidates.filter((c) => !this.history.slice(-policy.excludeRecent).includes(c));
