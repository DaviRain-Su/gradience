...
import { buildCreateTaskPermissionIx, deriveTaskPda, type MembersArgs } from '../settlement/per-program-builder.js';
import {
    address,
    getAddressEncoder,
    AccountRole,
...
import { resolveJudgeAndPayPdas, findApplicationPda } from '../solana/pda-resolver.js';
...
export class SettlementBridge extends EventEmitter {
     ...
     */
    async setupTaskPermission(taskIdOnChain: string, members: MembersArgs = { members: null }): Promise<string> {
     ...
     */
    async settleWithPER(request: SettlementRequest, score: number, reasonRef: string): Promise<SettlementResult> {
        ...
        try {
            ...
            const verificationHash = this.hashProofData({ reasonRef, score, settlementId } as unknown as Omit<
                EvaluationProof,
                'signature'
            >);
     ...
     */
    async settleWithReasonRef(request: SettlementRequest, score: number, reasonRef: string): Promise<SettlementResult> {
        const settlementId = `${request.evaluationId}-${Date.now()}`;
    ...
    private async submitToAgentArena(request: SettlementRequest, proof: EvaluationProof): Promise<SettlementResult> {
        logger.info(
            {
                taskId: request.taskId,
                agentId: request.agentId,
    ...
    createPaymentConfirmation(settlementResult: SettlementResult, request: SettlementRequest): PaymentConfirmation {
        return {
