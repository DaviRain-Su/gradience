...
import { SettlementBridge, createSettlementBridge } from './settlement-bridge.js';
...
describe('SettlementBridge', () => {
  ...
  describe('settlement', () => {
    it('should settle evaluation result', async () => {
      ...
      expect(agentAmount).toBe((total * 95n) / 100n);
      expect(judgeAmount).toBe((total * 3n) / 100n);
      expect(protocolAmount).toBe((total * 2n) / 100n);
    });
