...
export class BridgeManager {
    ...
    async initialize(): Promise<void> {
        ...
        logger.info({ evaluatorPubkey, autoSettle: this.config.autoSettle }, 'BridgeManager initialized');
    }
     ...
     */
    async submitExternalEvaluation(params: ExternalEvaluationParams): Promise<ExternalEvaluationResult> {
        this.ensureInitialized();

        logger.info({ taskId: params.taskId, evaluator: params.evaluatorAuthority }, 'Submitting external evaluation');
        ...
        if (result.success) {
            ...
            logger.error({ taskId: params.taskId, error: result.error }, 'External evaluation submission failed');
        }
    ...
    private ensureInitialized(): void {
        if (!this.config.enabled) {
            throw new DaemonError(ErrorCodes.INVALID_REQUEST, 'Bridge settlement not enabled', 400);
        }
        if (!this.initialized) {
            throw new DaemonError(ErrorCodes.INVALID_REQUEST, 'BridgeManager not initialized', 500);
        }
    }
}
