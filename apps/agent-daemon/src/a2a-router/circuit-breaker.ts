...
export class CircuitBreaker {
    ...
    constructor(name: string, config: Partial<CircuitBreakerConfig> = {}) {
        ...
        // Register metrics
        metrics.gauge(A2A_METRICS.CIRCUIT_BREAKER_STATE, `Circuit breaker state for ${name}`);
        metrics.counter(A2A_METRICS.CIRCUIT_BREAKER_TRANSITIONS, 'Circuit breaker state transitions');
    }
    ...
    private transitionTo(newState: CircuitState): void {
        ...
        metrics.set(A2A_METRICS.CIRCUIT_BREAKER_STATE, stateValue, { circuit: this.name });
        metrics.inc(A2A_METRICS.CIRCUIT_BREAKER_TRANSITIONS, 1, {
            circuit: this.name,
            from: oldState,
            to: newState,
        });
...
export class CircuitBreakerRegistry {
    ...
    hasOpenCircuit(): boolean {
        return Array.from(this.breakers.values()).some((breaker) => breaker.getState() === 'open');
    }
