...
export class A2ARouter {
  ...
  private options: Required<Pick<A2ARouterEnhancedOptions,
    'enableNostr' | 'enableXMTP' | 'healthCheckInterval' | 'messageTimeout' |
    'enableCircuitBreaker' | 'enableRateLimiting' | 'enableRetry' |
    'enableValidation' | 'enableMetrics' | 'enableHealthMonitor'>>;
        ...
        try {
          const subscription = await adapter.subscribe((message) => {
            ...
            if (this.options.enableValidation) {
              ...
              if (!validation.valid) {
                logger.warn({ error: validation.error, messageId: message.id }, 'Invalid message received');
                return;
              }
            }
  ...
  private selectProtocol(priority: ProtocolPriority): ProtocolType | null {
    ...
    const availableAdapter = Array.from(this.adapters.entries()).find(
  ...
  private startHealthMonitoring(): void {
    ...
    this.healthMonitor.registerCheck({
      ...
      check: async () => {
        ...
        const status = heapUsedMB > thresholdMB * 1.5
          ? 'unhealthy'
