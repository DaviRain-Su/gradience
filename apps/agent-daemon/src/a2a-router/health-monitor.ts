...
import type { ProtocolType, RouterHealthStatus, ProtocolHealthStatus } from '@gradiences/a2a-types';
...
export class HealthMonitor {
    ...
    constructor(config: Partial<HealthMonitorConfig> = {}) {
        ...
        // Register metrics
        metrics.gauge(A2A_METRICS.HEALTH_STATUS, 'Overall health status (0=healthy, 1=degraded, 2=unhealthy)');
        metrics.gauge(A2A_METRICS.HEALTH_CHECK_DURATION, 'Health check duration');
        metrics.counter(A2A_METRICS.HEALTH_CHECK_FAILURES, 'Health check failures');
    }
    ...
    start(): void {
        ...
        Array.from(this.checks.entries()).forEach(([name, registration]) => {
     ...
     */
    stop(): void {
        Array.from(this.checkIntervals.entries()).forEach(([name, interval]) => {
    ...
    getHealth(): SystemHealth {
        ...
        Array.from(this.results.entries()).forEach(([name, result]) => {
            checks[name] = result;
        });
        ...
        metrics.set(A2A_METRICS.HEALTH_STATUS, statusValue);
    ...
    private async executeCheck(registration: HealthCheckRegistration): Promise<HealthCheckResult> {
        ...
        try {
            ...
            // Update metrics
            metrics.set(A2A_METRICS.HEALTH_CHECK_DURATION, Date.now() - startTime, {
            ...
            metrics.inc(A2A_METRICS.HEALTH_CHECK_FAILURES, 1, { check: registration.name });
    ...
    private calculateOverallStatus(
        ...
        const hasCriticalFailure = Array.from(this.checks.entries()).some(([name, registration]) => {
            if (!registration.critical) return false;
