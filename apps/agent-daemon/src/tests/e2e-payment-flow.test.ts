...
describe('E2E Payment Flow', () => {
  ...
  describe('Complete Payment Flow', () => {
    it('should complete full payment flow: request → accept → evaluate → settle → confirm', async () => {
      ...
      const accepted = await paymentService.acceptPayment(
        session.paymentId,
        'e2e-payee'
      );
      ...
      const confirmation = settlementBridge.createPaymentConfirmation(
        settlementResult,
        {
          evaluationId,
          taskId: session.taskId,
      ...
      console.log(`   Distribution: ${settlementResult.distribution.agent} / ${settlementResult.distribution.judge} / ${settlementResult.distribution.protocol}`);
    });
  });
