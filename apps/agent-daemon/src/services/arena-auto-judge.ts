...
export class ArenaAutoJudgeService {
    ...
    private async processTask(task: TaskItem): Promise<void> {
        ...
        if (submissions.length < this.config.minSubmissions) {
            logger.info({ taskId: task.task_id, count: submissions.length }, 'Insufficient submissions, skipping');
            return;
        }
                    ...
                    categories: [
                        {
                            name: 'relevance',
                            weight: 0.4,
                            description: 'How relevant the submission is to the task',
                            criteria: ['Alignment with task requirements'],
                        },
                        {
                            name: 'quality',
                            weight: 0.4,
                        ...
                        },
                        {
                            name: 'completeness',
                            weight: 0.2,
                            description: 'Whether the submission covers all aspects',
                            criteria: ['All requirements addressed'],
                        },
                    ],
                },
                requiredChecks: [],
