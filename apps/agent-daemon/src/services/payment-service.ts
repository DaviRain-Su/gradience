...
export class PaymentService extends EventEmitter {
    ...
    async requestPayment(params: {
        ...
        if (!check.allowed) {
            throw new DaemonError(ErrorCodes.PAYMENT_NOT_ALLOWED, check.reason ?? 'Payment not allowed by policy', 403);
        }
    ...
    async acceptPayment(paymentId: string, payeeAgentId: string): Promise<PaymentSession> {
        ...
        if (session.payeeAgentId !== payeeAgentId) {
            throw new DaemonError(ErrorCodes.PAYMENT_UNAUTHORIZED, 'Agent is not the designated payee', 403);
        }

        // Get payee wallet
        const payeeWallet = this.walletManager.getWallet(payeeAgentId);
        if (!payeeWallet) {
            throw new DaemonError(ErrorCodes.WALLET_NOT_FOUND, `Payee wallet not found for agent ${payeeAgentId}`, 404);
        }
            ...
            try {
                ...
                switch (payload.type) {
                    case 'payment_request':
                        await this.handleIncomingPaymentRequest(message.from, payload.data as PaymentRequest);
                        break;
                    case 'payment_confirmation':
                        await this.handlePaymentConfirmation(message.from, payload.data as PaymentConfirmation);
                        break;
    ...
    private async sendDisputeNotification(session: PaymentSession, dispute: PaymentDispute): Promise<void> {
        const recipient = session.payerAgentId === dispute.initiator ? session.payeeAgentId : session.payerAgentId;
    ...
    private async handleIncomingPaymentRequest(from: string, request: PaymentRequest): Promise<void> {
        // Validate request
        validatePaymentRequest(request);

        logger.info({ paymentId: request.paymentId, from, amount: request.amount }, 'Received payment request');
    ...
    private async handlePaymentConfirmation(from: string, confirmation: PaymentConfirmation): Promise<void> {
    ...
    private async evaluateAndSettle(session: PaymentSession): Promise<void> {
        try {
            ...
            logger.info({ paymentId: session.paymentId, taskId: session.taskId }, 'Starting evaluation for payment');
    ...
    private createMockConfirmation(
        ...
        const txHash = requiresReview ? `pending-review-${Date.now()}` : `evaluated-${Date.now()}-${score}`;
