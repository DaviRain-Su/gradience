...
export class WorkflowProtectionService {
    ...
    verifyFingerprint(workflow: ProtectedWorkflow): boolean {
        ...
        const stepsHash = workflow.steps.map((s) => s.fingerprint).join('');

        const expected = hash(workflow.author + workflow.metadata.description + stepsHash);
    ...
    async detectCopy(
        ...
        for (const existing of existingWorkflows) {
            ...
            const matchingSteps = newWorkflow.steps.filter((newStep) =>
                existing.steps.some((existingStep) => existingStep.fingerprint === newStep.fingerprint),
            ).length;
