...
export class WorkflowExecutionEngine {
  ...
  constructor(
    db: Database,
    protectionService: WorkflowProtectionService,
    a2aRouter: any,
    x402Manager: any
  ) {
    this.db = db;
  ...
  async execute(ctx: ExecutionContext): Promise<ExecutionResult> {
    ...
    try {
      ...
      const cost = this.protectionService.calculateExecutionCost(
        workflow,
        ctx.selectedSteps
      );
      ...
      const actualCost = revenueDistribution.reduce(
        (sum, r) => sum + r.amount,
        0n
      );
  ...
  private async executeStep(
    ...
    try {
      ...
      if (step.visibility === 'encrypted' || step.visibility === 'protected') {
        // Decrypt step logic
        const decrypted = await this.protectionService.decryptStep(
          step,
          executionKey
        );
  ...
  private async distributeRevenue(
    distribution: ExecutionResult['revenueDistribution']
  ): Promise<void> {
  ...
  previewExecution(
    ...
    const stepsToShow = selectedSteps
