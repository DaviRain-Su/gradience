...
export class PolicyEngine {
    ...
    async evaluate(policies: PolicyDefinition[], ctx: SigningContext): Promise<PolicyEvaluation> {
        ...
        for (const policy of policies) {
            ...
            if (policy.executable) {
                try {
                    const execResult = await evaluateExecutable(policy.executable, ctx, policy.config ?? null);
                    if (!execResult.allow) {
                        results.push({
                            allowed: false,
                            policyId: policy.id,
