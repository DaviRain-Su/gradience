...
export class OWSWalletManager {
    ...
    async createWallet(params: {
        ...
        const existing = this.stmts!.getWalletByAgent.get(params.agentId) as { id: string } | undefined;
        if (existing) {
     ...
     */
    async updateReputation(agentId: string, newReputation: ReputationData, reason?: string): Promise<OWSWallet | null> {
        ...
        // Update wallet
        this.stmts!.updateReputation.run(newScore, JSON.stringify(newPolicy), now, wallet.id);

        // Record history
        this.stmts!.insertReputationHistory.run(wallet.id, oldScore, newScore, reason ?? 'reputation_update', now);
    ...
    async batchSyncReputations(parentWallet: string): Promise<{
        ...
        logger.info({ parentWallet, total: wallets.length, updated, failed }, 'Batch reputation sync completed');
    ...
    getDailySpend(walletId: string): number {
        ...
        const row = this.stmts!.getDailySpend.get(walletId, today) as { amount_usd_cents: number } | undefined;
    ...
    private static prepareStatements(db: Database.Database) {
      ...
      `),
            ...
            listWalletsByParent: db.prepare(
                `SELECT * FROM ows_wallets WHERE parent_wallet = ? ORDER BY created_at DESC`,
            ),
