...
export class HandshakeFSM extends EventEmitter {
    ...
    constructor(sessionId: string, localDid: string, config: Partial<P2pSoulConfig> = {}) {
        super();
    ...
    async transition(event: HandshakeEvent): Promise<HandshakeState> {
        ...
        if (!handler) {
            throw new HandshakeError(`Invalid transition: ${this.state} + ${event.type}`, this.state);
        }
...
export function createHandshakeFSM(sessionId: string, localDid: string, config?: Partial<P2pSoulConfig>): HandshakeFSM {
