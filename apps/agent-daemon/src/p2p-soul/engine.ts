...
import type { SoulProfile, SoulDigest, MatchEvaluation, MatchConfig, Verdict } from './types.js';
...
export class MatchEngine {
    ...
    private calculateSkillComplementarity(local: SoulProfile, remote: SoulDigest): number {
        ...
        const localHasRemoteNeeds = [...remoteSkillCategories].filter((cat) => !localSkillCategories.has(cat)).length;

        const remoteHasLocalNeeds = [...localSkillCategories].filter((cat) => !remoteSkillCategories.has(cat)).length;
    ...
    private calculateAvailabilityMatch(local: SoulProfile, remote: SoulDigest): number {
        ...
        const localAvailability =
            availabilityMap[
                local.availability.hoursPerWeek < 10 ? 'low' : local.availability.hoursPerWeek < 30 ? 'medium' : 'high'
            ];
     ...
     */
    private weightedAverage(scores: {
        interest: number;
        skill: number;
        reputation: number;
        availability: number;
 ...
 */
export function generateDisclosureData(profile: SoulProfile, rawSoulMd: string, level: DisclosureLevel): unknown {
    switch (level) {
