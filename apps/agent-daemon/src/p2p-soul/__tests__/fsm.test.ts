...
describe('HandshakeFSM', () => {
    ...
    describe('Invalid Transitions', () => {
        it('should throw on invalid transition', async () => {
            await expect(fsm.disclose(DisclosureLevel.LEVEL_1_ANONYMOUS, 'interested')).rejects.toThrow(
                'Invalid transition',
            );
        });

        it('should throw on confirmMatch from IDLE', async () => {
            await expect(fsm.confirmMatch()).rejects.toThrow('Invalid transition');
        });
    });
});
