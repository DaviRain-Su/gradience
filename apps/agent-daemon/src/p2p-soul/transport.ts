...
import type { SoulMessage, MessageType, X25519KeyPair, EncryptedData } from './types.js';
import {
...
export class P2PTransportManager extends EventEmitter {
    ...
    async sendMessage(toDid: string, message: SoulMessage, remotePublicKey?: Uint8Array): Promise<SendResult> {
    ...
    private async verifyMessageSignature(message: SoulMessage): Promise<boolean> {
        ...
        return verifySignature(Buffer.from(messageData), message.signature, senderPublicKey);
    }
    ...
    private async sendAcknowledgment(toDid: string, messageId: string): Promise<void> {
        ...
        ackMessage.signature = signMessage(Buffer.from(messageData), this.keyPair.privateKey);
