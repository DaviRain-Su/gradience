...
import type { X25519KeyPair, EncryptedData, MerkleProof, CryptoError } from './types.js';
 ...
 */
export async function encryptDisclosure(data: unknown, sharedSecret: Uint8Array): Promise<EncryptedData> {
 ...
 */
export function decryptDisclosure(encrypted: EncryptedData, sharedSecret: Uint8Array): unknown {
    const key = hkdfSha256(sharedSecret, 'soul-handshake-salt', 'encryption-key');
 ...
 * Verify a commitment
 */
export function verifyCommitment(commitment: string, data: unknown, nonce: string): boolean {
    const dataStr = typeof data === 'string' ? data : JSON.stringify(data);
