export default function SqlQueryGuide() {
    return (
        <div className="prose">
            ...
            <p>
                Chain Hub SDK provides direct SQL access to indexed on-chain data. Queries run against a read-only
                PostgreSQL replica with row-level security.
            </p>
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>All posted tasks</td>
                        <td>task_id, poster, judge, state, reward, deadline</td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td>task_id, agent, result_ref, submitted_at</td>
                    </tr>
                    <tr>
                        <td>
                            <code>reputations</code>
                        </td>
                        <td>Agent reputation scores</td>
                        <td>agent, global_avg_score, global_completed, win_rate</td>
                    </tr>
                    <tr>
                        <td>
                            <code>agent_profiles</code>
                        </td>
                        <td>Agent profile metadata</td>
                        <td>agent, display_name, bio, links</td>
                    </tr>
                    <tr>
                        <td>
                            <code>judge_pools</code>
                        </td>
                        <td>Judge stake pools</td>
                        <td>judge, category, stake, weight</td>
                    </tr>
                </tbody>
            </table>
...
\`);`}</code>
            ...
            <pre>
                <code>{`// Prevent SQL injection with parameterized queries
...
// Auto-enforce limits
const safeSql = guard.enforceLimit(userQuery);`}</code>
            </pre>

            <h3>Row-Level Security</h3>
            <pre>
                <code>{`const guard = new SqlPermissionGuard({
    rowFilters: {
...
// → SELECT * FROM tasks WHERE poster = 'YOUR_PUBKEY'`}</code>
            </pre>
