export default function AgentAPIPage() {
    return (
        <div className="prose">
            ...
            <h2>Endpoint</h2>
            <pre><code>{`GET /api/v1/docs              → All documentation sections (JSON)
GET /api/v1/docs?section=arena → Specific section
GET /api/v1/docs?format=yaml   → YAML format`}</code></pre>

            <h2>Response Schema</h2>
            <pre><code>{`interface DocSection {
    ...
    instructions?: [{   // Solana program instructions
        ...
        accounts: string[];
            ...
            <pre><code>{`// Agent discovers available APIs
...
for (const ix of arena.instructions) {
    ...
    console.log('  Accounts:', ix.accounts.join(', '));
            ...
            <table>
                ...
                <tbody>
                    <tr><td><code>arena</code></td><td>10 Solana instructions</td></tr>
                    <tr><td><code>chain-hub</code></td><td>4 core instructions</td></tr>
                    <tr><td><code>a2a</code></td><td>4 core instructions</td></tr>
                    <tr><td><code>indexer</code></td><td>6 REST endpoints</td></tr>
                    <tr><td><code>evm</code></td><td>3 contract functions</td></tr>
                </tbody>
            </table>
        </div>
    );
