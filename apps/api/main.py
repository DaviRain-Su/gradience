...
TEMPLATES_PATH = os.getenv("FASTAPI_TEMPLATES_PATH", os.path.join(os.path.dirname(__file__), "templates.json"))
...
if os.path.exists(DASHBOARD_STATIC_PATH):
    app.mount("/", StaticFiles(directory=DASHBOARD_STATIC_PATH, html=True), name="dashboard")
...
@app.get("/health")
async def health():
    uptime_sec = round(time.time() - datetime.fromisoformat(API_STARTED_AT.replace("Z", "")).timestamp(), 2)
    errors = []
...
async def health_node():
    ...
    try:
        with urllib.request.urlopen(f"{NODE_API_URL}/api/templates") as response:
            ...
            return {"status": "ok", "nodeStatus": response.status, "latencyMs": latency_ms}
    except urllib.error.URLError as exc:
        raise HTTPException(status_code=502, detail=f"Node API unreachable: {exc}")
