...
use crate::{
    ...
    utils::{
        ...
        find_stake_pda, find_task_pda, get_judge_pool, get_program_config, get_stake, get_task,
        TestContext,
    },
};

#[test]
fn t19d_force_refund_judge_kept_in_pool_when_stake_sufficient() {
...
#[test]
fn t19d_force_refund_removes_judge_when_stake_insufficient() {
    let mut ctx = TestContext::new();
    let poster = ctx.create_funded_keypair();
    let judge = ctx.create_funded_keypair();
    let anyone = ctx.create_funded_keypair();
...
#[test]
fn t19d_upgrade_config_updates_min_stake_and_enforces_authority() {
    let mut ctx = TestContext::new();
