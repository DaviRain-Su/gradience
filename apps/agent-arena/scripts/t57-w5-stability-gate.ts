...
import {
    runT47PoolAltDrill,
    type T47DrillReport,
    type T47InstructionBaseline,
} from './t47-pool-alt.ts';
...
export function buildChecklist(
    ...
    const abnormalPathsStatus: T57ChecklistStatus = !t46Report
        ? 'not_executed'
        : t46Report.ok
          ? 'pass'
          : 'fail';
    ...
    const eventLoopStatus: T57ChecklistStatus =
        wsPrimary === null || wsReconnect === null
            ? 'not_executed'
            : wsPrimary && wsReconnect
              ? 'pass'
              : 'fail';
...
export function evaluateT57Gate(input: {
    ...
    for (const phase of input.phases) {
        if (phase.required && phase.status !== 'pass') {
            failures.push(
                `${phase.id} failed (${phase.status})${phase.error ? `: ${phase.error}` : ''}`,
            );
        }
    }
    ...
    const availableBaselines = new Set(
        (input.t47Report?.instructionBaselines ?? []).map((item) => item.instruction),
    );
    ...
    if (input.t48Report && !input.allowUnknownMode) {
        if (input.t48Report.modeClassification === 'unknown') {
            failures.push(
                `unknown T48 judge mode classification: ${input.t48Report.activeJudgeMode}`,
            );
        }
    }
...
export async function runT57W5StabilityGate(
    options: T57StabilityGateOptions = {},
): Promise<T57StabilityReport> {
    ...
    const t46 = await runPhase('t46', 'T46 abnormal paths', true, () =>
...
if (isMainEntry) {
    ...
    const allowUnknownMode =
        argv.includes('--allow-unknown-mode') ||
        parseBoolean(process.env.T57_ALLOW_UNKNOWN_MODE, false);
