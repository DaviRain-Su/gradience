...
use crate::{
    ...
    events::{TaskRefundedEvent, TASK_REFUND_REASON_EXPIRED},
    instructions::RefundExpired,
    state::{
        ACCOUNT_VERSION_V1, APPLICATION_DISCRIMINATOR, ESCROW_DISCRIMINATOR, TASK_DISCRIMINATOR,
        Application, Escrow, Task, TaskState,
    },
    traits::EventSerialize,
    utils::{
        borsh_deserialize_padded, emit_event, token_program_kind as resolve_token_program_kind, validate_mint_and_get_decimals,
...
pub fn process_refund_expired(
    program_id: &Address,
    accounts: &[AccountView],
    instruction_data: &[u8],
) -> ProgramResult {
    let ix = RefundExpired::try_from((instruction_data, accounts))?;
    ...
    if clock.unix_timestamp <= task.deadline {
        return Err(GradienceProgramError::DeadlineNotPassed.into());
    }
    ...
    if let Some(token_accounts) = token_path_accounts {
        ...
        transfer_from_escrow_token(&transfer_ctx, token_accounts.poster_token_account, task.reward)?;
        ...
        verify_system_account(poster)?;
        if address_to_bytes(poster.address()) != task.poster {
            return Err(ProgramError::InvalidInstructionData);
        }

        transfer_program_lamports(ix.accounts.escrow, poster, task.reward)?;
    ...
    let event = TaskRefundedEvent {
        ...
        reason: TASK_REFUND_REASON_EXPIRED,
        amount: task.reward,
    };
    emit_event(
        program_id,
