...
export class GradienceSDK {
    ...
    readonly reputation = {
         ...
         */
        get: (agent: Address) => this.getReputationOnChain(agent),
    } as const;
    ...
    private async resolveJudgeTokenContext(params: {
        mint?: Address;
        tokenProgram?: Address;
        escrow: Address;
        judge: Address;
    ...
    private async resolveForceRefundTokenContext(params: {
        mint?: Address;
        tokenProgram?: Address;
        escrow: Address;
        poster: Address;
    ...
    private async resolveRefundPairs(
        taskId: bigint,
...
async function findApplicationPda(
    programAddress: Address,
    taskId: bigint,
    agent: Address,
): Promise<readonly [Address, number]> {
...
async function findSubmissionPda(
    programAddress: Address,
    taskId: bigint,
    agent: Address,
): Promise<readonly [Address, number]> {
...
async function findTreasuryPda(programAddress: Address): Promise<readonly [Address, number]> {
    return getProgramDerivedAddress({
        programAddress,
...
function addressToBytes(value: Address | undefined): number[] {
    if (!value) {
        return new Array(32).fill(0);
    }
    return Array.from(getAddressEncoder().encode(value) as unknown as Uint8Array);
}
...
function stripOptionalTail<TInstruction extends Instruction>(
...
function removeAccountsAtIndexes<TInstruction extends Instruction>(
    instruction: TInstruction,
    indexes: number[],
): TInstruction {
...
function appendRemainingAccounts<TInstruction extends Instruction>(
    instruction: TInstruction,
...
function parseReputationAccount(data: Uint8Array): ReputationOnChain {
    ...
    const byCategory: ReputationCategoryOnChain[] = [];
