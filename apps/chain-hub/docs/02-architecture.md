...
```mermaid
flowchart TB
    AIM["Agent.im (用户入口)"] --> SDK["Chain Hub SDK"]
...
**前端入口**：Chain Hub 不自建独立 UI。用户通过 Agent.im "发现"页浏览技能市场、管理委托任务。Agent 通过 A2A API（localhost:3939）或直接调用 Chain Hub SDK 操作技能和委托。
...
2. **发现**：Agent.im 用户/Agent 通过技能市场浏览
...
**当前实现状态**：链上 SkillRegistry 完整（注册/启停），SDK invoke 完整（REST/CPI 路由），Agent.im 前端技能市场页面待实现（P2）。
