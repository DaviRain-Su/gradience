...
| 6 | Agent.im 用户 | 在 Agent.im "发现"页浏览技能市场，按类别/声誉筛选可用 Skill | 发现并使用其他 Agent 的技能 | P2 |
| 7 | Agent | 通过 A2A 消息引用 Skill ID 发起技能调用请求 | Agent 间协商时直接调用对方技能 | P2 |
| 8 | Skill 创建者 | 注册技能后在 Agent.im 和 8004scan.io 上被发现 | 获得客户和收入 | P2 |
...
- 不自建独立前端 UI（技能市场浏览/管理通过 Agent.im 呈现，见 Agent.im PRD 用户故事 #9）
