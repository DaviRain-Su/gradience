...
pub struct RecordDelegationExecutionData {
    pub task_id: u64,
    pub execution_ref_hash: [u8; 32],
}

pub fn process_record_delegation_execution(
    program_id: &Address,
    accounts: &[AccountView],
    instruction_data: &[u8],
) -> ProgramResult {
    let data = RecordDelegationExecutionData::try_from_slice(instruction_data)
        .map_err(|_| ProgramError::InvalidInstructionData)?;

    let [agent, delegation_task_account] = accounts else {
    ...
    if agent.address().to_bytes() != task.selected_agent_authority {
        return Err(ChainHubError::UnauthorizedAgent.into());
    }
