...
pub struct CompleteDelegationTaskData {
    pub task_id: u64,
}

pub fn process_complete_delegation_task(
    program_id: &Address,
    accounts: &[AccountView],
    instruction_data: &[u8],
) -> ProgramResult {
    let data = CompleteDelegationTaskData::try_from_slice(instruction_data)
        .map_err(|_| ProgramError::InvalidInstructionData)?;

    let [actor, delegation_task_account] = accounts else {
    ...
    let actor_key = actor.address().to_bytes();
    if actor_key != task.requester && actor_key != task.selected_judge_authority {
        return Err(ChainHubError::UnauthorizedJudge.into());
    }
