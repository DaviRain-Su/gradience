...
export class A2ASdk {
  ...
  async assignSubtaskBid(input: {
    ...
    return this.config.transport.send(
      buildInstruction(
        "assignSubtaskBid",
        [writableSigner(input.requester), writable(subtask), writable(bid)],
        {
          parentTaskId: input.parentTaskId.toString(),
          subtaskId: input.subtaskId,
          winner: input.winner,
  ...
  async submitSubtaskDelivery(input: {
    ...
    return this.config.transport.send(
      buildInstruction(
        "submitSubtaskDelivery",
        [writableSigner(input.selectedAgent), writable(subtask)],
        {
          parentTaskId: input.parentTaskId.toString(),
          subtaskId: input.subtaskId,
  ...
  async cancelSubtaskOrder(input: {
    requester: Address;
    parentTaskId: bigint;
    subtaskId: number;
  }): Promise<string> {
    ...
    return this.config.transport.send(
      buildInstruction(
        "cancelSubtaskOrder",
        [writableSigner(input.requester), writable(subtask)],
        {
          parentTaskId: input.parentTaskId.toString(),
          subtaskId: input.subtaskId,
        },
  ...
  getThread(
    ...
    counterparty: Address,
    threadId: bigint,
  ): Promise<MessageThreadAccount | null> {
    return this.config.transport.getAccount(
      threadPda(this.config.programId, creator, counterparty, threadId),
    );
  }
  ...
  getChannel(
    ...
    payee: Address,
    channelId: bigint,
  ): Promise<PaymentChannelAccount | null> {
    return this.config.transport.getAccount(
      channelPda(this.config.programId, payer, payee, channelId),
    );
  }
  ...
  getSubtask(
    parentTaskId: bigint,
    subtaskId: number,
  ): Promise<SubtaskOrderAccount | null> {
    return this.config.transport.getAccount(
      subtaskPda(this.config.programId, parentTaskId, subtaskId),
    );
  }
  ...
  getSubtaskBid(
    ...
    subtaskId: number,
    bidder: Address,
  ): Promise<SubtaskBidAccount | null> {
    return this.config.transport.getAccount(
      bidPda(this.config.programId, parentTaskId, subtaskId, bidder),
    );
  }
}
