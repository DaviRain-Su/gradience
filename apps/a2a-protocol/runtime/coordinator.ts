...
export class CoordinatorService {
    ...
    async decomposeTask(taskId: string, manualSubtasks?: Partial<SubtaskSpec>[]): Promise<SubtaskSpec[]> {
    ...
    async receiveBid(taskId: string, subtaskId: number, bid: Omit<BidSummary, 'score'>): Promise<void> {
        const task = this.tasks.get(taskId);
        if (!task) throw new Error(`Task not found: ${taskId}`);
    ...
    async aggregateResults(
        ...
        const avgQuality =
            Object.keys(subtaskResults).length > 0 ? totalQuality / Object.keys(subtaskResults).length : 0;
