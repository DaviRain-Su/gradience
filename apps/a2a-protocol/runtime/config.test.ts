...
import { relayProfileDefaults, resolveRelayRuntimeConfig } from './config';
...
test('relay profile defaults are stable for devnet and prod', () => {
    ...
    assert.equal(devnet.alertThresholds.criticalDbFailureRate >= devnet.alertThresholds.maxDbFailureRate, true);
    ...
    assert.equal(prod.alertThresholds.criticalDbAvgQueryLatencyMs >= prod.alertThresholds.maxDbAvgQueryLatencyMs, true);
...
test('relay runtime config resolves profile and env overrides', () => {
    ...
    assert.equal(resolved.alertSlackWebhookUrl, 'https://hooks.slack.com/services/test');
    ...
    assert.equal(resolved.postgresConnectionString, 'postgres://localhost:5432/a2a');
