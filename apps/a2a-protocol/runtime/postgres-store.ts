...
export class PostgresRelayStore implements RelayStore {
    ...
    async upsertAgent(descriptor: Omit<RelayAgentDescriptor, 'heartbeatAt'>): Promise<RelayAgentDescriptor> {
    ...
    async publishEnvelope(envelope: SignedEnvelope, body: Record<string, unknown>): Promise<RelayEnvelopeRecord> {
        return this.withState(async (state) => {
    ...
    async pullEnvelopes(agent: Address, afterId?: string, limit = 100): Promise<RelayPullResult> {
        return this.withState(async (state) => {
            state.metrics.pullRequests += 1;
            const startIndex =
                afterId === undefined
                    ? 0
                    : Math.max(0, state.envelopes.findIndex((item) => item.envelope.id === afterId) + 1);
    ...
    async getMetrics(): Promise<RelayMetrics> {
        ...
        return {
            ...
            dbAvgQueryLatencyMs: this.dbQueryCount === 0 ? 0 : this.dbQueryLatencyTotalMs / this.dbQueryCount,
        };
    }
    ...
    private async withState<T>(mutation: (state: PersistedRelayState) => Promise<T> | T): Promise<T> {
        const state = await this.readState();
    ...
    private async assertRolePolicy(): Promise<void> {
       ...
       WHERE rolname = current_user`,
        ...
        const roleName = typeof row.role_name === 'string' && row.role_name.trim() !== '' ? row.role_name : 'unknown';
        throw new Error(
    ...
    private async readState(): Promise<PersistedRelayState> {
        ...
        const table = this.tableName;
        const result = await this.query(`SELECT state_json FROM ${table} WHERE singleton_key = $1`, [
            this.singletonKey,
        ]);
        const row = result.rows[0];
        const json = row && typeof row.state_json === 'string' ? row.state_json : JSON.stringify(DEFAULT_STATE);
        const parsed = JSON.parse(json) as PersistedRelayState;
        if (parsed.version !== 1) {
            throw new Error(`unsupported relay state version: ${String((parsed as { version?: number }).version)}`);
        }
    ...
    private async writeState(state: PersistedRelayState): Promise<void> {
        await this.readyPromise;
        const table = this.tableName;
        await this.query(`UPDATE ${table} SET state_json = $1, updated_at = NOW() WHERE singleton_key = $2`, [
...
async function createPgPool(connectionString: string, options: PostgresPoolOptions = {}): Promise<SqlClientLike> {
...
function normalizePersistedDbAlertState(state: Partial<RelayDbAlertState> | undefined): RelayDbAlertState {
    return {
...
function isRolePolicyError(error: unknown): boolean {
    return error instanceof Error && error.message.includes('requires non-superuser, non-createdb, non-createrole');
}

export function buildPgPoolConfig(connectionString: string, options: PostgresPoolOptions = {}): PgPoolConfig {
    return {
        connectionString,
...
function deserializeEnvelopeRecord(item: PersistedRelayState['envelopes'][number]): RelayEnvelopeRecord {
    return {
        envelope: {
            id: item.envelope.id,
...
function serializeEnvelopeRecord(item: RelayEnvelopeRecord): PersistedRelayState['envelopes'][number] {
    return {
        envelope: {
            id: item.envelope.id,
