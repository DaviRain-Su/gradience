...
import { evaluateRelayAlerts, RelayAlertMonitor, type RelayAlertThresholds } from './monitor';
...
test('evaluateRelayAlerts emits expected warnings', () => {
    ...
    assert.deepEqual(alerts.map((item) => item.code).sort(), [
...
test('RelayAlertMonitor deduplicates repeated DB incident alerts with cooldown and emits on escalation', async () => {
    ...
    assert.equal(
        second.alerts.some((item) => item.code === 'db_query_failure_rate_high'),
        false,
    );
    const third = await monitor.checkNow();
    assert.equal(
        third.alerts.some((item) => item.code === 'db_query_failure_rate_high'),
        false,
    );
    ...
    assert.equal(
        dbAlerts.some((item) => item.severity === 'critical'),
        true,
    );
});
