import { DEFAULT_RELAY_ALERT_THRESHOLDS, type RelayAlertThresholds } from './monitor';
...
export function resolveRelayRuntimeConfig(env: Record<string, string | undefined>): RelayRuntimeConfig {
    ...
    return {
        ...
        transportEncryptionKey:
            parseOptionalString(env.A2A_RELAY_TRANSPORT_ENCRYPTION_KEY) ?? preset.transportEncryptionKey,
        maxPayloadBytes: parseIntSafe(env.A2A_RELAY_MAX_PAYLOAD_BYTES, preset.maxPayloadBytes),
        maxPaymentMicrolamports: BigInt(
            parseIntSafe(env.A2A_RELAY_MAX_PAYMENT_MICROLAMPORTS, Number(preset.maxPaymentMicrolamports)),
        ),
        httpMaxBodyBytes: parseIntSafe(env.A2A_RELAY_HTTP_MAX_BODY_BYTES, preset.httpMaxBodyBytes),
        ...
        postgresConnectionString: parseOptionalString(env.A2A_RELAY_POSTGRES_URL) ?? preset.postgresConnectionString,
        postgresRejectElevatedRole: parseBoolean(
            env.A2A_RELAY_POSTGRES_REJECT_ELEVATED_ROLE,
            preset.postgresRejectElevatedRole,
        ),
        postgresRequireSsl: parseBoolean(env.A2A_RELAY_POSTGRES_REQUIRE_SSL, preset.postgresRequireSsl),
        ...
        alertIntervalMs: parseIntSafe(env.A2A_RELAY_ALERT_INTERVAL_MS, preset.alertIntervalMs),
        alertWebhookUrl: parseOptionalString(env.A2A_RELAY_ALERT_WEBHOOK_URL) ?? preset.alertWebhookUrl,
        alertSlackWebhookUrl: parseOptionalString(env.A2A_RELAY_ALERT_SLACK_WEBHOOK_URL) ?? preset.alertSlackWebhookUrl,
        alertMinSeverity: parseSeverity(env.A2A_RELAY_ALERT_MIN_SEVERITY, preset.alertMinSeverity),
        alertDispatchCooldownMs: parseIntSafe(env.A2A_RELAY_ALERT_DISPATCH_COOLDOWN_MS, preset.alertDispatchCooldownMs),
        alertRetryAttempts: parseIntSafe(env.A2A_RELAY_ALERT_RETRY_ATTEMPTS, preset.alertRetryAttempts),
        alertRetryBaseDelayMs: parseIntSafe(env.A2A_RELAY_ALERT_RETRY_BASE_DELAY_MS, preset.alertRetryBaseDelayMs),
        alertSigningSecret: parseOptionalString(env.A2A_RELAY_ALERT_SIGNING_SECRET) ?? preset.alertSigningSecret,
        alertFailureQueueFilePath:
            parseOptionalString(env.A2A_RELAY_ALERT_FAILURE_QUEUE_FILE) ?? preset.alertFailureQueueFilePath,
        alertReplayOnStart: parseBoolean(env.A2A_RELAY_ALERT_REPLAY_ON_START, preset.alertReplayOnStart),
        alertThresholds: {
            maxRejectedPayloads: parseIntSafe(
                env.A2A_RELAY_ALERT_MAX_REJECTED_PAYLOADS,
                preset.alertThresholds.maxRejectedPayloads,
            ),
            maxDedupRatio: parseFloatSafe(env.A2A_RELAY_ALERT_MAX_DEDUP_RATIO, preset.alertThresholds.maxDedupRatio),
...
export function relayProfileDefaults(profile: RelayRuntimeProfile): RelayRuntimeConfig {
    const preset = PROFILE_PRESETS[profile];
    return {
        ...preset,
...
function parsePositiveIntAtLeastOne(input: string | undefined, fallback: number): number {
...
function parseSeverity(input: string | undefined, fallback: RelayAlertSeverity): RelayAlertSeverity {
...
export const DEFAULT_ALERT_THRESHOLDS: RelayAlertThresholds = DEFAULT_RELAY_ALERT_THRESHOLDS;
