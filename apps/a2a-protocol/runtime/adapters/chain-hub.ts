...
export interface ChainHubClient {
    recordDelegationExecution(input: { taskId: bigint; executionRefHash: string }): Promise<string>;
    completeDelegationTask(input: { taskId: bigint }): Promise<string>;
}

export class ChainHubAdapter {
    constructor(private readonly client: ChainHubClient) {}

    verifyPolicyHash(input: { expectedPolicyHash: string; actualPolicyHash: string }): void {
