...
test('orchestrator assignLowestBid picks smallest quote', async () => {
    const orchestrator = new A2AOrchestrator(mockProgramClient(), new A2ARelayApi(new InMemoryRelayStore()), {
...
test('orchestrator retries transient program failures', async () => {
    ...
    const orchestrator = new A2AOrchestrator(flakyProgram, new A2ARelayApi(new InMemoryRelayStore()), {
