...
test('relay server exposes health and relay endpoints', async () => {
    ...
    try {
        ...
        try {
            ...
            const pull = await fetch(`${server.baseUrl}/v1/envelopes/pull?agent=agent-b&limit=1`, {
                headers: {
                    authorization: 'Bearer token',
                },
            });
            ...
            const replayUnauthorized = await fetch(`${server.baseUrl}/v1/alerts/replay-failed`, {
                method: 'POST',
                headers: {
                    'content-type': 'application/json',
                },
