...
export interface RelayAlertSinkOptions {
    ...
    postJson?: (url: string, payload: unknown, headers: Record<string, string>) => Promise<void>;
}
...
export class RelayAlertSink {
    ...
    private readonly postJson: (url: string, payload: unknown, headers: Record<string, string>) => Promise<void>;
    ...
    isEnabled(): boolean {
        return Boolean(normalizeUrl(this.options.webhookUrl) || normalizeUrl(this.options.slackWebhookUrl));
    }
    ...
    async notify(alerts: RelayAlert[], metrics: RelayMetrics): Promise<void> {
        ...
        if (slackWebhookUrl) {
            requests.push(
                this.dispatchWithRetry(slackWebhookUrl, buildSlackPayload(this.source, candidates), sentAtTimestamp),
            );
        }
    ...
    private async dispatchWithRetry(url: string, payload: unknown, sentAtTimestamp: number): Promise<void> {
    ...
    private makeRequestHeaders(payload: unknown, sentAtTimestamp: number): Record<string, string> {
        ...
        headers['x-a2a-alert-signature'] = createAlertSignature(this.signingSecret, sentAtTimestamp, payload);
        return headers;
    }
...
export function createAlertSignature(secret: string, timestampMs: number, payload: unknown): string {
...
async function defaultPostJson(url: string, payload: unknown, headers: Record<string, string>): Promise<void> {
