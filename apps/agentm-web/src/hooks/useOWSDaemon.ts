...
export function useOWSDaemon() {
  ...
  const createWallet = useCallback(async (name: string, passphrase?: string) => {
    setError(null);
    try {
  ...
  const exportWallet = useCallback(async (nameOrId: string, passphrase?: string) => {
    try {
  ...
  const importMnemonic = useCallback(async (name: string, mnemonic: string, passphrase?: string) => {
    try {
  ...
  const createApiKey = useCallback(async (params: {
    name: string;
    walletIds: string[];
    policyIds: string[];
    passphrase: string;
