    ...
    try {
      ...
      try {
        ...
        const followersRes = await fetch(
          `${daemonUrl}/api/social/followers/${walletAddress}/count`,
      ...
      try {
        ...
        const followingRes = await fetch(
          `${daemonUrl}/api/social/following/${walletAddress}/count`,
          { headers: authHeaders(sessionToken) }
        );
        if (followingRes.ok) {
      ...
      try {
        ...
        const profileRes = await fetch(
          `${daemonUrl}/api/profile/${walletAddress}`,
          { headers: authHeaders(sessionToken) }
        );
        if (profileRes.ok) {
      ...
      try {
        ...
        const feedRes = await fetch(
          `${daemonUrl}/api/v1/social/feed/${walletAddress}?limit=1`,
          { headers: authHeaders(sessionToken) }
        );
        if (feedRes.ok) {
