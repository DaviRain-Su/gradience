...
export function useAgentProfiles(): UseAgentProfilesResult {
  ...
  const createProfile = useCallback(async (draft: ProfileDraft): Promise<AgentProfile | null> => {
  ...
  const updateProfile = useCallback(async (id: string, updates: Partial<AgentProfile>): Promise<AgentProfile | null> => {
    if (!walletAddress) return null;
  ...
  const updateProfileStatus = useCallback(async (id: string, status: AgentProfile['status']): Promise<boolean> => {
