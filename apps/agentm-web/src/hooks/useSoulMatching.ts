...
export function useSoulMatching(options: UseSoulMatchingOptions): UseSoulMatchingReturn {
  ...
  const calculateCommunicationMatch = (
    comm1: SoulProfile['communication'],
    comm2: SoulProfile['communication']
  ) => {
    let score = 0;
...
function generateDimensionAnalysis(
  ...
  const dimensionContent: Record<string, { summary: string; evidence: string[]; risks: string[]; suggestions: string[] }> = {
    values: {
