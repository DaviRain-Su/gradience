...
export function useERC8004() {
    ...
    async (agentURI: string): Promise<boolean> => {
      try {
        const response = await fetch(
          `${daemonUrl}/api/v1/erc8004/lookup?uri=${encodeURIComponent(agentURI)}`,
          { headers: getHeaders() }
        );
        return response.ok;
      } catch {
  ...
  const getAgentInfo = useCallback(
    async (agentId: string): Promise<{
      ...
      try {
        const response = await fetch(
          `${daemonUrl}/api/v1/erc8004/agents/${encodeURIComponent(agentId)}`,
          { headers: getHeaders() }
        );
