...
export function useIdentity() {
  ...
  const getBinding = useCallback(
    async (wallet: string): Promise<BindingInfo | null> => {
      if (!fetchApi) return null;
      return fetchApi<BindingInfo>(
        `/api/v1/identity/binding/${encodeURIComponent(wallet)}`
      );
    },
    [fetchApi]
  );
  ...
  const verifyZkKyc = useCallback(
    async (accountId: string, nullifierHash: string): Promise<{ accountId: string; zkVerified: boolean; nullifierHash: string } | null> => {
      if (!fetchApi) return null;
      return fetchApi<{ accountId: string; zkVerified: boolean; nullifierHash: string }>('/api/v1/identity/zk-verify', {
        method: 'POST',
