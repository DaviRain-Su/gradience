    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button
                    onClick={onClose}
                    style={{ ...btnPrimary, background: '#F3F3F8', color: '#16161A', border: '1.5px solid #16161A' }}
                >
                    Back
                </button>
            </div>
            ...
            <div style={cardStyle}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '12px' }}>
                    <div>
                        <span style={{ opacity: 0.5 }}>Reward:</span> <strong>{rewardDisplay}</strong>
                    </div>
                    <div>
                            ...
                            >
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span style={{ fontSize: '12px', fontFamily: 'monospace' }}>
                                        {sub.agent.slice(0, 8)}...{sub.agent.slice(-4)}
                                    </span>
                                ...
                                <p style={{ fontSize: '12px', color: '#16161A', opacity: 0.7, marginTop: '4px' }}>
                                    Result: {sub.result_ref.slice(0, 40)}
                                    {sub.result_ref.length > 40 ? '...' : ''}
                                </p>
            ...
            {isJudge && submissions.length > 0 && isOpen && (
                <div style={cardStyle}>
                    ...
                    <div style={{ marginBottom: '8px' }}>
                        ...
                        <span style={{ fontSize: '12px', fontFamily: 'monospace' }}>
                            {selectedWinner
                                ? `${selectedWinner.slice(0, 8)}...${selectedWinner.slice(-4)}`
                                : 'Click a submission above'}
                        </span>
                    </div>
                    ...
                    >
                        <input type="checkbox" checked={usePER} onChange={(e) => setUsePER(e.target.checked)} />
                    ...
                    >
                        {arena.loading
                            ? 'Sending...'
                            : usePER
                              ? `Judge & Settle via PER (${rewardDisplay})`
                              : `Judge & Settle (${rewardDisplay})`}
                    </button>
                </div>
            )}
        </div>
