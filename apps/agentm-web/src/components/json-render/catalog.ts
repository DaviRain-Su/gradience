...
export const agentmCatalog = (defineCatalog as any)(schema, {
    ...
    actions: {
        submitConfig: {
            ...
            params: z.object({ config: z.record(z.string(), z.unknown(), 'config') }, 'submitConfig params'),
        },
        updateField: {
            description: '更新字段值',
            params: z.object({ field: z.string(), value: z.unknown() }, 'updateField params'),
        },
        refreshData: { description: '刷新数据', params: z.object({}, 'refreshData params') },
        validateForm: {
            description: '验证表单',
            params: z.object({ fields: z.array(z.string()) }, 'validateForm params'),
        },
...
export function validateAgentmSpec(spec: unknown): { valid: true } | { valid: false; errors: string[] } {
    ...
    if (!s.root || typeof s.root !== 'string')
        return { valid: false, errors: ["Spec must have a 'root' string property"] };
    if (!s.elements || typeof s.elements !== 'object')
        return { valid: false, errors: ["Spec must have an 'elements' object property"] };
    ...
    for (const [id, element] of Object.entries(elements)) {
        ...
        if (!el.type || typeof el.type !== 'string') {
            errors.push(`Element '${id}' must have a 'type' property`);
            continue;
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const catalogComponents = (agentmCatalog.data as any)?.components;
        if (!catalogComponents?.[el.type])
            errors.push(
                `Element '${id}' has unknown type '${el.type}'. Available types: ${componentTypeList.join(', ')}`,
            );
        if (el.props !== undefined && typeof el.props !== 'object')
            ...
            if (!Array.isArray(el.children)) errors.push(`Element '${id}' children must be an array`);
                ...
                for (const child of el.children) {
                    if (typeof child !== 'string') {
                        errors.push(`Element '${id}' children must be element IDs (strings)`);
                        break;
                    }
                    if (!elements[child]) errors.push(`Element '${id}' references unknown child '${child}'`);
                }
        }
    }
