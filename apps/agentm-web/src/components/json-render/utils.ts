...
import { agentmCatalog, componentTypeList, actionTypeList, validateAgentmSpec } from './catalog';
...
export function generateCatalogDescription(): string {
    ...
    const basicComponents = ['Card', 'Button', 'Input', 'Select', 'Slider', 'Switch', 'Textarea', 'Label'];
    ...
    const customComponents = ['TokenSelector', 'PriceChart', 'MetricCard', 'AddressInput', 'AmountInput'];
...
// 生成 Spec 模板
export function generateSpecTemplate(type: SpecGenerationContext['type']): Spec {
...
// 生成示例 Spec
export function generateExampleSpec(example: 'token-monitor' | 'price-alert' | 'simple-form'): Spec {
    switch (example) {
        case 'token-monitor':
            return {
                root: 'main-card',
...
// 提取 Spec 中的所有字段
export function extractFieldsFromSpec(spec: Spec): Array<{ id: string; type: AgentmComponentType; label?: string }> {
...
export function getSpecSummary(spec: Spec): {
    ...
    function calculateDepth(elementId: string, visited = new Set<string>()): number {
        ...
        const childDepths = element.children.map((childId) => calculateDepth(childId, new Set(visited)));
...
// 创建渲染上下文
export function createRenderContext(config: RenderConfig = {}): {
