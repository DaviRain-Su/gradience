 ...
 * 使用自然语言生成 Agent 配置界面
 */
export function SmartConfig({ initialPrompt = '', onComplete, onCancel, className = '' }: SmartConfigProps) {
        ...
        <div className={`space-y-6 ${className}`}>
            ...
            <div className="flex gap-2 border-b">
                <button
                    ...
                    className={`px-4 py-2 font-medium ${
                        mode === 'input' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground'
                    }`}
                >
                    智能配置
                </button>
                <button
                    ...
                    disabled={!spec}
                    className={`px-4 py-2 font-medium ${
                        mode === 'preview' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground'
            ...
            {mode === 'input' ? (
                <div className="space-y-4">
                    {/* 提示词输入 */}
                    <div>
                        <label className="block text-sm font-medium mb-2">描述你的 Agent 需求</label>
                        <textarea
                            value={prompt}
                    ...
                    {/* 错误状态 */}
                    {error && <JsonRenderError error={error} onRetry={generateSpec} />}
                </div>
            ) : (
                <div className="space-y-4">
                    {/* 预览模式 */}
                    {spec && (
                        <>
                            <div className="p-4 border rounded-lg bg-card">
                                <JsonRender spec={spec} initialData={formData} onChange={handleFormChange} />
                            </div>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={handleBack} className="px-4 py-2 border rounded-md hover:bg-muted">
                                    返回修改
                                </button>
                            </div>
                        </>
