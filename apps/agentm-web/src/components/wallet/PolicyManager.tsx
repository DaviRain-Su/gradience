...
const POLICY_TEMPLATES = [
  {
    ...
    rules: [
      { type: 'allowed_chains', chain_ids: ['solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1'] },
    ],
    dailyLimitSol: 0.1,
  },
  {
        ...
        {showCreate && (
          <div style={{ background: '#F3F3F8', borderRadius: '8px', padding: '16px', marginBottom: '12px' }}>
            <input
              ...
              style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #16161A', marginBottom: '12px', fontSize: '13px' }}
            />
            
            {/* Chain Selection */}
            <p style={{ fontSize: '12px', fontWeight: 600, color: '#16161A', marginBottom: '8px' }}>Allowed Chains:</p>
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '12px' }}>
              {CHAIN_OPTIONS.map(chain => (
                <label key={chain.id} style={{ fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                  <input
                    type="checkbox"
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '12px' }}>
              <label style={{ fontSize: '12px', color: '#16161A', minWidth: '120px' }}>Daily Limit (SOL):</label>
              <input
                type="number"
                step="0.01"
                min="0"
                ...
                placeholder="0.1"
                style={{ width: '100px', padding: '6px 10px', borderRadius: '6px', border: '1px solid #16161A', fontSize: '12px' }}
              />
            </div>
            
            {/* GRA-224: Task-only mode */}
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '12px' }}>
              <label style={{ fontSize: '12px', color: '#16161A', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                <input
                  type="checkbox"
            ...
            {/* Expiry */}
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '12px' }}>
              <label style={{ fontSize: '12px', color: '#16161A', minWidth: '120px' }}>Expires in (days):</label>
              <input
                type="number"
                value={expiryDays}
                onChange={e => setExpiryDays(e.target.value)}
                placeholder="30"
                style={{ width: '80px', padding: '6px 10px', borderRadius: '6px', border: '1px solid #16161A', fontSize: '12px' }}
              />
            </div>
        ...
        {policies.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {policies.map(p => {
              ...
              const dailyLimitRule = p.rules?.find((r: any) => r.type === 'daily_spend_limit') as { lamports: number } | undefined;
              const dailyLimit = dailyLimitRule ? (dailyLimitRule.lamports / 1_000_000_000).toFixed(2) : null;
              const allowedPrograms = p.rules?.find((r: any) => r.type === 'allowed_programs') as { programs: string[] } | undefined;
                ...
                <div key={p.id} style={{
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '11px', color: '#16161A', opacity: 0.6 }}>
                      ...
                      {!dailyLimit && !isTaskOnly && (p.rules?.map((r: any) => r.type).join(', ') || 'No rules')}
                    </p>
                  </div>
                  <button
      ...
      <div style={cardStyle}>
        ...
        {showCreateKey && (
          <div style={{ background: '#F3F3F8', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
            ...
            {/* Wallet Selection */}
            <p style={{ fontSize: '12px', fontWeight: 600, color: '#16161A', marginBottom: '8px' }}>Select Wallets:</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '12px' }}>
              {wallets.length === 0 ? (
                <p style={{ fontSize: '11px', color: '#dc2626' }}>No wallets available. Create a wallet first.</p>
              ) : (
                wallets.map(w => (
                  <label key={w.id} style={{ fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                    <input
                      type="checkbox"
            ...
            <p style={{ fontSize: '12px', fontWeight: 600, color: '#16161A', marginBottom: '8px' }}>Attach Policies:</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '12px' }}>
              {policies.length === 0 ? (
                <p style={{ fontSize: '11px', color: '#666' }}>No policies available. Create policies first for better security.</p>
              ) : (
                policies.map(p => (
                  <label key={p.id} style={{ fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                    <input
                      type="checkbox"
            ...
            <button onClick={handleCreateApiKey} style={btnPrimary} disabled={!keyName.trim() || selectedWallets.length === 0 || !keyPassphrase}>
              Create API Key
            </button>
          </div>
        )}
        ...
        {createdKeyToken && (
          <div style={{ background: '#fef3c7', borderRadius: '8px', padding: '12px', marginBottom: '16px', border: '1px solid #f59e0b' }}>
                ...
                <div>
                  ...
                  {k.policyIds.length > 0 && (
                    <p style={{ fontSize: '10px', color: '#666', marginTop: '4px' }}>
                      Policies: {k.policyIds.map(pid => policies.find(p => p.id === pid)?.name || pid).join(', ')}
                    </p>
                  )}
                </div>
                <button
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '400px', overflowY: 'auto' }}>
              {auditLog.map((entry: any, i: number) => (
                <div key={i} style={{
                  ...
                  {entry.context && (
                    <p style={{ fontSize: '11px', color: '#16161A', opacity: 0.7, marginBottom: '4px' }}>
                      {entry.context.operation} on {entry.context.chain}
                      {entry.context.amount && ` | Amount: ${(entry.context.amount / 1_000_000_000).toFixed(4)} SOL`}
                    </p>
                  )}
