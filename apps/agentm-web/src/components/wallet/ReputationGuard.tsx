    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
        ...
        <div>
          ...
          <p style={{ fontSize: '18px', fontWeight: 700, color: COLORS.ink, margin: 0 }}>{transaction.token}</p>
        </div>
        <div>
        ...
        {transaction.recipient && (
          <div>
            <p style={{ fontSize: '12px', color: COLORS.ink, opacity: 0.5, margin: '0 0 4px 0' }}>Recipient</p>
            <p style={{ fontSize: '13px', color: COLORS.ink, opacity: 0.8, fontFamily: 'monospace', margin: 0 }}>
      ...
      {checks.map((check) => (
        <div key={check.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            {check.note && <p style={{ fontSize: '12px', color: COLORS.ink, opacity: 0.4, margin: '2px 0 0 0' }}>{check.note}</p>}
          </div>
      ...
      {checkResult?.allowed && checkResult.requiresApproval && (
        ...
        >
          <p style={{ fontSize: '14px', color: COLORS.warning, margin: 0 }}>
            Your reputation tier requires manual approval for transactions. Increase your reputation
            score to {tier === 'bronze' ? 31 : 51}+ to enable automatic transactions.
          </p>
        </div>
      )}
...
export function ReputationGuardBadge({
  ...
  return (
    <button
      ...
      title={
        checkResult?.reason ?? (status === 'approval' ? 'Approval required' : 'Transaction allowed')
      }
      onMouseEnter={(e) => {
