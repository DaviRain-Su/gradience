...
export function MasterReputationCard({ masterWallet }: MasterReputationCardProps) {
    ...
    if (!masterWallet) {
        return (
            <div style={cardStyle}>
                <p style={{ fontSize: '14px', color: '#666' }}>Connect your wallet to view reputation</p>
            </div>
        );
    }
    ...
    if (error || !aggregated) {
        return (
            <div style={cardStyle}>
                <p style={{ fontSize: '14px', color: '#dc2626' }}>{error || 'Failed to load reputation'}</p>
            </div>
        );
    }
        ...
        <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                ...
                <div>
                    ...
                    <p style={{ fontSize: '12px', color: '#666', margin: '4px 0 0 0' }}>Master Wallet Reputation</p>
                </div>
            ...
            >
                <StatBox label="Agents" value={aggregated.agentCount} color="#16161A" />
                <StatBox label="Tasks Done" value={aggregated.totalCompletedTasks} color="#16161A" />
                <StatBox label="Daily Limit" value={`$${aggregated.derivedPolicy.dailyLimitUsd}`} color="#22c55e" />
            </div>

            {/* Permissions */}
            <div style={{ background: '#F3F3F8', borderRadius: '8px', padding: '12px' }}>
                <p style={{ fontSize: '12px', fontWeight: 600, color: '#16161A', margin: '0 0 8px 0' }}>Permissions</p>
                            ...
                            return (
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        ...
                                        <span style={{ fontSize: '13px', color: '#16161A' }}>{agent.handle}</span>
                                    </div>
...
function StatBox({ label, value, color }: { label: string; value: string | number; color: string }) {
    return (
        <div style={{ textAlign: 'center', padding: '12px', background: '#F3F3F8', borderRadius: '8px' }}>
            <p style={{ fontSize: '18px', fontWeight: 700, color, margin: '0 0 4px 0' }}>{value}</p>
