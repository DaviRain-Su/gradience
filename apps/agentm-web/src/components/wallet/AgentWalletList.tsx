 ...
 */
export function AgentWalletList({ masterWallet, activeWalletId, onSelectWallet }: AgentWalletListProps) {
    const { wallets, loading, error } = useOWSDaemon();

    if (!masterWallet) {
        return (
            <div style={cardStyle}>
                <p style={{ fontSize: '14px', color: '#666' }}>Connect wallet to view agent wallets</p>
            </div>
        );
    }
    ...
    return (
        <div style={cardStyle}>
            ...
            >
                <h3 style={{ fontSize: '16px', fontWeight: 600, color: '#16161A', margin: 0 }}>Agent Wallets</h3>
                <span style={{ fontSize: '12px', color: '#666' }}>{walletsWithReputation.length} wallets</span>
            </div>
...
function TierLegend({ emoji, label, color }: { emoji: string; label: string; color: string }) {
    return (
