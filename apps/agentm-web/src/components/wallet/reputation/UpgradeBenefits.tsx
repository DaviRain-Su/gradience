    ...
    return (
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
          ...
          <h4 style={{ fontSize: '14px', fontWeight: 600, color: COLORS.ink, margin: 0 }}>
            Platinum Status
          </h4>
        </div>
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <span>
            <span style={{ color: COLORS.ink, opacity: 0.4 }}>{formatUsdCents(currentPolicy.dailyLimit)}</span>
            <span style={{ margin: '0 4px', color: COLORS.ink, opacity: 0.4 }}>→</span>
            <span style={{ color: nextConfig.color, fontWeight: 600 }}>{formatUsdCents(nextPolicy.dailyLimit)}</span>
          </span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ color: COLORS.ink, opacity: 0.6 }}>Max TX</span>
          <span>
            <span style={{ color: COLORS.ink, opacity: 0.4 }}>{formatUsdCents(currentPolicy.maxTransaction)}</span>
            <span style={{ margin: '0 4px', color: COLORS.ink, opacity: 0.4 }}>→</span>
            <span style={{ color: nextConfig.color, fontWeight: 600 }}>{formatUsdCents(nextPolicy.maxTransaction)}</span>
          </span>
        </div>
