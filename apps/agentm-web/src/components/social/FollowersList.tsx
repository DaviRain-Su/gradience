...
export function FollowersList({
    ...
    return (
      <div style={{ background: c.surface, borderRadius: '16px', border: `1.5px solid ${c.ink}`, padding: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
          <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '18px', fontWeight: 700, margin: 0 }}>Followers</h3>
  ...
  if (error) {
    return (
      <div style={{ background: c.surface, borderRadius: '16px', border: `1.5px solid ${c.ink}`, padding: '16px' }}>
        <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '18px', fontWeight: 700, margin: '0 0 12px 0' }}>Followers</h3>
            ...
            {followers.map((follower) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    {follower.avatarUrl ? (
                      ...
                      <div style={{ width: '40px', height: '40px', background: c.lime, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: 700, border: `1.5px solid ${c.ink}` }}>
                    ...
                    <div>
                      ...
                      {follower.displayName && (
                        <p style={{ fontSize: '12px', opacity: 0.5, margin: '2px 0 0 0', fontFamily: 'monospace' }}>{follower.address.slice(0, 16)}...</p>
                      )}
                ...
                {expandedFollower === follower.address && follower.followedAt && (
                  <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: `1px dashed ${c.bg}`, fontSize: '12px', opacity: 0.5 }}>
                    <p style={{ margin: 0 }}>Followed since: {new Date(follower.followedAt).toLocaleDateString()}</p>
                  </div>
                )}
              </div>
            ))}
