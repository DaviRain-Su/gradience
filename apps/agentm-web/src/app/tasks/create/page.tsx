...
export default function CreateArenaTaskPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (chain !== "evm") {
      ...
      minStakeValue = minStake
        ? BigInt(Math.round(parseFloat(minStake) * 1e9))
        : undefined;
    }
      ...
      >
        ...
        >
          ...
          <p style={{ margin: "4px 0 0", fontSize: "14px", color: "#666" }}>
            Create an on-chain task with escrow. Reward will be locked until a
            judge settles the task.
          </p>
        </div>
        ...
        >
                ...
                <MagicBlockStatus
                  sessions={[
                    ...magicBlock.sessions.map((s) => ({
                      id: s.id,
                      mode: s.mode,
                      status: (s.state === 'active' ? 'active' : 'connecting') as import("@/components/settlement/MagicBlockStatus").SessionStatus,
          ...
          >
            Settlement split: <strong>95%</strong> winner /{" "}
            <strong>3%</strong> judge / <strong>2%</strong> protocol
          </div>
          ...
          >
            {loading ? "Posting task..." : walletAddress ? `Post Task & Lock ${tokenSymbol}` : "Connect wallet to post"}
          </button>
        </form>
      </div>
    </div>
