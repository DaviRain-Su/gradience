    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 20px' }}>
            ...
            {stats && (
                ...
                >
                    <StatCard title="Total Agents" value={stats.stats.totalAgents.toLocaleString()} icon="🤖" />
                    <StatCard title="Avg Reputation" value={stats.stats.avgReputationScore} icon="⭐" />
                    <StatCard
                        title="Tasks Completed"
                        value={stats.stats.totalTasksCompleted.toLocaleString()}
                        icon="✅"
                    />
                    <StatCard title="Oracle Version" value={stats.gradienceOracle.version} icon="🔄" />
                </div>
            )}
            ...
            {stats && (
                <div style={cardStyle}>
                    <h2 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 20px 0' }}>Tier Distribution</h2>
                    <div
                        style={{
                            display: 'grid',
                    ...
                    >
                        <TierCard
                            ...
                            minScore={80}
                        />
                        <TierCard tier="Gold" count={stats.stats.tierDistribution.gold} color="#FFD700" minScore={60} />
                        <TierCard
                            tier="Silver"
            ...
            {stats && (
                <div style={{ ...cardStyle, marginTop: '20px' }}>
                    <h2 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 20px 0' }}>🔗 Connected Registries</h2>
                        ...
                        {stats.connectedRegistries.map((registry) => (
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p style={{ fontSize: '12px', color: '#666', margin: 0 }}>{registry.type}</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {stats && (
                <div style={{ ...cardStyle, marginTop: '20px' }}>
                    <h2 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 20px 0' }}>📊 Data Sources</h2>
                    <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                        {stats.dataSources.map((source) => (
                            ...
                            >
                                <span style={{ marginRight: '8px' }}>{source.status === 'active' ? '🟢' : '🔴'}</span>
                                {source.name}
                            </div>
                        ))}
                    </div>
            ...
            {/* Verify Agent */}
            <div style={{ ...cardStyle, marginTop: '20px' }}>
                <h2 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 20px 0' }}>🔍 Verify Agent Reputation</h2>
            ...
            {/* Leaderboard */}
            <div style={{ ...cardStyle, marginTop: '20px' }}>
                <h2 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 20px 0' }}>🏆 Top Agents by Reputation</h2>
            ...
            {/* API Documentation */}
            <div style={{ ...cardStyle, marginTop: '20px' }}>
                <h2 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 20px 0' }}>📚 API Documentation</h2>
                ...
                >
                    ...
                    <p style={{ margin: 0 }}>GET /api/v1/oracle/reputation/&#123;agentAddress&#125;</p>
                </div>
...
function TierCard({ tier, count, color, minScore }: { tier: string; count: number; color: string; minScore: number }) {
    return (
        <div
            style={{
                padding: '20px',
        ...
        >
            <p style={{ fontSize: '14px', color: '#666', margin: '0 0 4px 0' }}>{minScore}+ Score</p>
            <p style={{ fontSize: '24px', fontWeight: 700, margin: '0 0 4px 0' }}>{count}</p>
            <p style={{ fontSize: '14px', fontWeight: 600, color, margin: 0 }}>{tier}</p>
        </div>
    );
}
