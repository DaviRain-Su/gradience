...
function EVMProfileRegistrationCard({
    ...
    return (
        <div style={{ marginTop: '24px', paddingTop: '24px', borderTop: `1px solid ${c.ink}` }}>
            <h3 style={{ fontSize: '16px', fontWeight: 700, color: c.ink, marginBottom: '8px' }}>
                EVM Profile Registration
            </h3>
            {registered === true ? (
            ...
            if (isEvmWalletConnected && primaryWallet?.address) {
                ...
                try {
                    ...
                    if (!registered) {
                        setToastMessage({
                            type: 'error',
                            message: 'Please register your Gradience profile on EVM first (Domain tab).',
                        });
                        setSubmitting(false);
                        return;
                    }
            ...
            if (editingAgentProfile) {
                ...
                setToastMessage({
                    ...
                    message: onChainResult
                        ? 'Agent profile updated and published to EVM'
                ...
                setToastMessage({
                    type: 'success',
                    message: onChainResult
                        ? 'Agent profile created and published to EVM'
                    ...
                    >
                        ...
                        {isEvmWalletConnected && (
                            <EVMProfileRegistrationCard
                                ...
                                bio={profile.bio}
                                onRegistered={() =>
                                    setToastMessage({ type: 'success', message: 'EVM profile registered successfully' })
                                }
                            />
                        )}
                    </div>
