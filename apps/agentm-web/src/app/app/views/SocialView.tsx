...
// Use first demo profile as "current user" for matching
...
export function SocialView({ address }: { address: string | null }) {
    ...
    const otherProfiles = useMemo(() => demoProfiles.filter(p => p.id !== MY_PROFILE.id), []);

    const matches = useMemo<ProfileMatch[]>(() => {
        return otherProfiles
            .map(profile => ({
                profile,
                result: calculateCompatibility(MY_PROFILE, profile),
            }))
            .sort((a, b) => b.result.score - a.result.score);
    }, [otherProfiles]);

    const handleViewProfile = (profile: SoulProfile) => {
        const match = matches.find(m => m.profile.id === profile.id);
        setSelectedProfile(profile);
        setSelectedMatch(match?.result ?? null);
    };
    ...
    return (
        <div style={{ display: 'flex', height: '100%', background: colors.bg, padding: '24px', gap: '24px' }}>
                ...
                <div style={{ background: colors.surface, borderRadius: '24px', padding: '20px', border: `1.5px solid ${colors.ink}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: `1px dashed ${colors.ink}`, paddingBottom: '12px', marginBottom: '12px' }}>
                        <div>
                            ...
                            <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: '32px', fontWeight: 700, lineHeight: 1 }}>{matches.length}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                            <div style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', opacity: 0.6 }}>Avg Score</div>
                            <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: '32px', fontWeight: 700, lineHeight: 1 }}>
                                {matches.length > 0 ? Math.round(matches.reduce((s, m) => s + m.result.score, 0) / matches.length) : 0}
                            </div>
                        </div>
                    </div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                        {[{ icon: '\ud83c\udfaf', label: 'Top Match', value: matches[0] ? `${Math.round(matches[0].result.score)}%` : '-', bg: colors.lime },
                          { icon: '\ud83d\udc8e', label: 'Shared Values', value: `${matches.reduce((s, m) => s + m.result.sharedValues.length, 0)}`, bg: colors.lavender },
                        ].map(item => (
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: colors.surface, borderRadius: '16px', padding: '8px', border: `1.5px solid ${colors.ink}` }}>
                    <div style={{ display: 'flex', gap: '4px' }}>
                        {[
                            { id: 'discover' as SocialTab, label: '\ud83d\udd0d Discover', count: otherProfiles.length },
                            { id: 'matches' as SocialTab, label: '\ud83d\udc95 Matches', count: matches.length },
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '16px' }}>
                            {otherProfiles.map(profile => (
                                <SoulProfileCard key={profile.id} profile={profile as any}
                                    ...
                                    onStartProbe={() => handleViewProfile(profile)}
                                />
                            ))}
                        </div>
                    )}
                    ...
                    {activeTab === 'matches' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            {matches.map(({ profile, result }) => (
                                <MatchCard key={profile.id} profile={profile} result={result} onClick={() => handleViewProfile(profile)} />
                            ))}
                        </div>
                    )}
            ...
            {/* Profile Modal */}
            {selectedProfile && (
                <ProfileModal profile={selectedProfile} match={selectedMatch} onClose={() => { setSelectedProfile(null); setSelectedMatch(null); }} />
            )}
        </div>
    );
}

function MatchCard({ profile, result, onClick }: { profile: SoulProfile; result: MatchResult; onClick: () => void }) {
    ...
    return (
        <div onClick={onClick} style={{
            ...
            display: 'flex', alignItems: 'center', gap: '20px', cursor: 'pointer', transition: 'transform 0.15s',
        }}>
            <div style={{
            ...
            <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: '6px', marginTop: '8px', flexWrap: 'wrap' }}>
                    ...
                    {result.sharedInterests.slice(0, 2).map(v => (
            ...
            <div style={{ textAlign: 'center' }}>
                <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: '36px', fontWeight: 700, color: colors.ink }}>
...
function ProfileModal({ profile, match, onClose }: { profile: SoulProfile; match: MatchResult | null; onClose: () => void }) {
    return (
        <div onClick={onClose} style={{
            ...
            <div onClick={e => e.stopPropagation()} style={{
                                ...
                                {[
                                    ...
                                    <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                        ...
                                        <div style={{ flex: 1, height: '8px', background: `${colors.ink}15`, borderRadius: '4px' }}>
                                            <div style={{ width: `${item.score}%`, height: '100%', background: item.color, borderRadius: '4px', transition: 'width 0.5s' }} />
                                        </div>
                ...
                <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '24px', position: 'relative' }}>
                    ...
                    <Section title="About">
                        <p style={{ color: colors.ink, opacity: 0.8, lineHeight: 1.6 }}>{profile.identity.bio}</p>
                    </Section>

                    <Section title="Core Values">
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                            {profile.values.core.map((v, i) => {
                    ...
                    <Section title="Interests">
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                            {profile.interests.topics.map((topic, i) => {
                                const isShared = match?.sharedInterests.map(s => s.toLowerCase()).includes(topic.toLowerCase());
                                return <span key={i} style={{ padding: '6px 12px', background: isShared ? `${colors.lime}60` : colors.bg, borderRadius: '8px', fontSize: '12px', border: `1.5px solid ${colors.ink}` }}>#{topic}{isShared ? ' \u2714' : ''}</span>;
                            })}
                        </div>
                    </Section>
                    ...
                    <Section title="Communication Style">
                        <div style={{ display: 'flex', gap: '16px' }}>
                            {[{ label: 'Tone', value: profile.communication.tone, bg: colors.lavender },
                              { label: 'Pace', value: profile.communication.pace, bg: colors.lime },
                              { label: 'Depth', value: profile.communication.depth, bg: colors.surface },
                            ].map(item => (
